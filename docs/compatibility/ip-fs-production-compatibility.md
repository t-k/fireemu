...
| AUTH-CREDENTIAL | ID/refresh/custom tokens, session cookies, signature/issuer/audience/project/tenant/expiry/revocation and claim composition across client/Admin/Rules | Finite saved production token flows; local session-cookie duration, signature, namespace, revocation and custom/session-claim handoff use signed local and unsigned-emulator trust roots plus pinned Admin SDK 14.3.0 | `IMPLEMENTING` | Production session-cookie/custom-token comparison, remaining claim precedence and SDK/Admin/Rules handoffs are required. |
...
| FS-DATA-WRITE | CRUD, BatchGet/List, Commit, BatchWrite, Write stream, values, masks, preconditions, transforms, limits, atomicity and post-state | Broad saved production comparisons; first46/second45 and G0 keep original and repaired results; local REST/gRPC covers Standard values, missing/null masks, preconditions, exact document-size and nesting boundaries with refusal post-state | `IMPLEMENTING` | Write stream, remaining request/operation limits and final-runtime saved-reference evaluation must be connected without weakening bindings. |
| FS-QUERY-INDEX | Filters, projection, sort, cursor, offset, limit, collection group, aggregation, vector, PartitionQuery, Explain and index acceptance/refusal | The current artifact replays 23/23 saved production aggregation observations; local PartitionQuery/Explain controls and bounded Standard vector execution cover REST/gRPC, transactions and index admission | `IMPLEMENTING` | Standard vector/index behavior, Explain, partitions and uncovered filter/aggregation conditions need representative production comparison. |
