"""Publish the local-versus-production comparison of the auth-mfa-start-disabled corpus.
...
that produced it, and the two are compared row by row on their semantic projection.
"""
...
RECEIPT = ROOT / "spec/compatibility/evidence/auth-mfa-start-disabled/receipt.json"
BUNDLE = (
    ROOT / "spec/compatibility/evidence/auth-mfa-start-disabled/local-comparison.json"
)
PAGE = ROOT / "docs/compatibility/auth-mfa-start-disabled-comparison.md"
...
RECORDER_FILES = (
    ...
    "tools/auth-mfa-start-disabled/start_disabled_owned.py",
...
if __name__ == "__main__":
    ...
    print("Auth mfa-start-disabled comparison checked; subject " + digest(value))
