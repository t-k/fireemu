...
use fireemu_core_app_check::claims::{issuer_for, AppCheckClaims};
use fireemu_core_app_check::jwt::{base64url_encode, encode};
use fireemu_core_app_check::registry::{AppCheckRegistry, AppRegistration, ProjectEpoch};
use fireemu_core_app_check::verify::{
...
fn a_forged_signature_a_foreign_key_and_an_unknown_key_id_are_all_refused() {
    ...
    let header = format!(
        ...
        <TestSigner as fireemu_core_app_check::crypto::AppCheckSigner>::kid(&signer)
    );
    ...
    let forged = format!(
        "{signing_input}.{}",
        base64url_encode(&fireemu_core_app_check::crypto::AppCheckSigner::sign(
            &foreign,
