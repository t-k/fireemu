...
The [current artifact-bound evidence](../../docs/compatibility/aggregation-evidence.md) was reacquired separately using the unchanged ten-case corpus. Both production and the rebuilt owned strict runtime return count 2 / sum 30.5 for `missing-before-limit`; the historical expected sum remains 10, so both receipts truthfully record 9/10 matches. Approvals remain empty. The [previous evidence bundle](../../spec/compatibility/evidence/history/aggregation-376db0d9/) is preserved byte-for-byte, rather than rewritten to fit the new implementation.

The next gate is an explicit, reviewed revision of the mistaken corpus expectation, followed by fresh artifact-bound receipts and approval of only the supported scope. These bounded observations do not establish general aggregation compatibility or authorize automatic promotion of candidate evidence.
