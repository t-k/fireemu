"""Observe revision-3 MFA pending usability and retain recovery journals on failure.
...
PROBE_TREES = (
    "tools/auth-pending-lifetime-window",
    "tools/auth-pending-lifetime",
    "tools/auth-pending-revocation",
    "tools/auth-password-maximum",
    "tools/compat-inventory",
)
# The declared observation budget. Revision 3 samples through 600 seconds. Verified
# administrator credentials and phase-specific refresh reserves remain mandatory. totalBudgetSeconds covers the largest age plus
# setup and a cleanup reserve for configuration restore and owned-account deletion;
...
def observe(output, origin=None, clock_control=None):
    ...
    try:
        ...
        def post_refusal(age, account, refused, old_credential):
            prefix = f"age-{age}s-post-refusal-"
            if not refused:
                for suffix in ("state", "fresh-start", "fresh-finalize"):
                    skipped(prefix + suffix)
                return
            read_sent = elapsed_now()
            status, response = admin("lookup", {"localId": [account["uid"]]})
            read_recv = elapsed_now()
            records = response.get("users", []) if status == 200 else []
            record = (
                records[0]
            ...
            checks = {
                "uidMatches": record.get("localId") == account["uid"],
                "emailMatches": record.get("email") == account["email"],
                "markerMatches": record.get("displayName") == account["marker"],
                "enabled": bool(record) and record.get("disabled", False) is False,
                "emailVerified": record.get("emailVerified") is True,
                "enrollmentMatches": enrollment.get("mfaEnrollmentId")
                == account["enrollmentId"],
                "phoneMatches": enrollment.get("phoneInfo") == account["phone"],
            }
            row(
                prefix + "state",
                status,
                response,
                checks,
                {"readbackSent": read_sent, "readbackReceived": read_recv},
            )
            if status != 200 or not all(checks.values()):
                skipped(prefix + "fresh-start")
                skipped(prefix + "fresh-finalize")
                return
            credential, p_sent, p_recv = timed_pending(account)
            require(credential != old_credential)
            status, started, s_sent, s_recv = timed_start(account, credential)
            session = (
            ...
            present = isinstance(session, str) and bool(session)
            row(
                prefix + "fresh-start",
                status,
                started,
                {"sessionInfoPresent": present},
                start_timing(p_sent, p_recv, s_sent, s_recv),
            )
            if not present:
                skipped(prefix + "fresh-finalize")
                return
            status, signed, f_sent, f_recv = timed_finalize(
                credential, session, code_for(session)
            )
            row(
                prefix + "fresh-finalize",
                status,
                signed,
        ...
        for a in sorted(AGE_SECONDS):
            ...
            refused = started_row["outcome"] == "refused"
            ...
            post_refusal(a, account, refused, entry["credential"])
