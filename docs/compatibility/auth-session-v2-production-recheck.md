...
The publisher now has a frozen-receipt validation path. `validate()` binds newly generated records to the current source inputs and runtime tree; `validate_frozen()` verifies an immutable receipt against its recorded probe and runtime snapshots. This prevents later unrelated source changes from invalidating historical evidence while retaining the artifact, contract and mutation checks. The current-source path remains strict for new observations.
