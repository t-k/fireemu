...
pub fn capabilities_manifest() -> Value {
    json!({
        ...
        "capabilities": {
            "CTL-1": {"status": "partial", "implemented": ["clock:set", "clock:advance", "clock:advanceTo", "health"], "unimplemented": ["sessions", "snapshots", "awaitIdle", "faultPlan"]},
            "FS-GW-1": {"status": "implemented", "precision": "conservative"},
            "FS-LIM-1": {"status": "partial", "implemented": ["FS-LIMIT-DOCUMENT-BYTES", "FS-LIMIT-NESTED-MAP-ARRAY-DEPTH", "FS-LIMIT-FIELD-TRANSFORMS-PER-DOCUMENT", "FS-LIMIT-COLLECTION-ID", "FS-LIMIT-DOCUMENT-ID", "FS-LIMIT-TRANSACTION-TOTAL-TIME"]},
            "FS-RPC-1": {"status": "partial", "implemented": ["GetDocument", "ListDocuments", "CreateDocument", "UpdateDocument", "DeleteDocument", "BatchGetDocuments", "BeginTransaction", "Commit", "Rollback", "RunQuery", "RunAggregationQuery", "ListCollectionIds", "BatchWrite"], "unimplemented": ["Listen", "Write", "PartitionQuery", "ExecutePipeline", "read_time consistency", "ListDocuments page tokens"]},
            "FS-REST-1": {"status": "unimplemented"},
            "FS-QRY-1": {"status": "implemented", "notes": ["orderBy on missing fields excludes documents", "range filters are type-restricted"]},
            "FS-TXN-1": {"status": "implemented", "precision": "boundary-conformance"},
            "FS-LSN-1": {"status": "unimplemented"},
            "FS-PIPE-RPC-1": {"status": "unimplemented"},
            "FS-TEXT-VAL-1": {"status": "unimplemented"},
            "AUTH-CORE-1": {"status": "partial", "implemented": ["accounts:signUp", "accounts:signInWithPassword", "accounts:lookup", "accounts:update", "securetoken token"], "unimplemented": ["anonymous sign-in via REST", "custom token", "delete"]},
            ...
            "AUTH-RULES-1": {"status": "partial", "notes": ["request.auth built from claims; Rules are not yet enforced on Firestore requests"]},
            ...
            "RULES-1": {"status": "partial", "notes": ["native evaluator subset; get()/exists() unsupported; not wired to the request path"]},
