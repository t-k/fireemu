...
def contract():
    path = Path(__file__).with_name("password_contract.py")
    assert path.exists(), "Password observation contract is required"
    spec = importlib.util.spec_from_file_location("password_contract", path)
...
def test_token_expiry_partitions_and_secret_redaction(seconds):
    ...
    c.validate_case(row, "change-password")
    assert row["passed"] is (seconds == "3600")
    assert "SECRET" not in json.dumps(row)
    with pytest.raises(ValueError):
        c.validate_case({**row, "passed": not row["passed"]}, "change-password")
    with pytest.raises(ValueError):
        c.validate_case({**row, "password": "SECRET"}, "change-password")
