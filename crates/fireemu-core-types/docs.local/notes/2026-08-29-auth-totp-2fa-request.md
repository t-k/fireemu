...
公式Firebase Emulator SuiteのAuth EmulatorはTOTPによる2要素認証（multi-factor TOTP）を再現できず不満点である。fireemuで解決できるとよい。

## 仕様書との関係

- `docs.local/fireemu-spec-v0.3.1-ja.md` 1.3「非目標」に「Firebase Auth、Realtime Database、Hosting、Remote Config等の再実装」が明記されており、Auth全体が初期scope外。
...
- TOTP secret生成、`otpauth://` URI、RFC 6238検証（time step 30s、window、SHA-1、6桁）を仮想時計（`Clock` trait）で決定的にする設計。仮想時計との整合はfireemuの強み。
