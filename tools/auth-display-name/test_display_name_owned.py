...
@pytest.mark.skipif(
    os.getenv("AUTH_DISPLAY_NAME_LIVE_LOCAL") != "1", reason="owned process opt-in"
)
def test_display_name_owned_lifecycle(tmp_path):
    path = Path(__file__).with_name("display_name_owned.py")
    ...
    from display_name_recorder import recovery_identity
