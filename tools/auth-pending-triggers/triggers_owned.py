"""Build and own the strict Auth artifact; run the same pending-triggers corpus against
it. Never attaches to an external daemon."""
...
from triggers_contract import TRIGGERS
from triggers_recorder import (
    PROJECT,
    ROOT,
    complete,
    inputs,
...
def child(directory, nonce):
    ...
    return observe(TRIGGER, directory / "observation", origin)
...
def run(output):
    ...
    process = None
    with tempfile.TemporaryDirectory(prefix="fireemu-triggers-owned-") as temporary:
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise SystemExit("Triggers owned observation did not complete") from None
