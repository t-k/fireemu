// Request guard and corpus validation of the AUTH-ACTION sandbox harness. Nothing here changes
// how a row is sent or recorded: the guard refuses a request before it leaves the process, and
// the validator refuses a corpus before a run starts. This module is therefore not part of the
...
// and mails nothing. The client `accounts:sendOobCode` is a PASSWORD_RESET for an address that
// has no account, which production answers without sending anything, or (owner decision
// 2026-09-24) one VERIFY_AND_CHANGE_EMAIL per token kind towards an unused @example.com
// address. Applying an email change may send the old address a change notice; every address is
// @example.com, which publishes a null MX (RFC 7505), so nothing is delivered.
...
function guardOobRequest(path, init, body, project) {
  ...
  if (clientEmailChange(body)) return;
  if (
...
/**
 * The one client email change the owner allowed (2026-09-24): VERIFY_AND_CHANGE_EMAIL for the
 * account behind an ID token, towards an `lvc-*` address no step ever gives an account. Its
 * confirmation mail goes to @example.com, which has a null MX.
 */
function clientEmailChange(body) {
  const keys = Object.keys(body).toSorted().join(",");
  return (
    body.requestType === "VERIFY_AND_CHANGE_EMAIL" &&
    keys === "idToken,newEmail,requestType" &&
    typeof body.idToken === "string" &&
    /^fireemu-aa-\d+-lvc-[a-z0-9-]+@example\.com$/.test(body.newEmail ?? "")
  );
}
  ...
  programs.forEach((program, index) => {
    ...
    for (const step of program.steps) {
      ...
      for (const key of ["email", "newEmail"]) {
        ...
        if (creates && !clientChange && address.startsWith("EMAIL(lvc-"))
          throw new Error(`${step.id}: an lvc-* address must never belong to an account`);
        if (creates && address.startsWith("EMAIL(unknown"))
