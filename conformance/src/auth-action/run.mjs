// AUTH-ACTION sandbox runner.
//
//   node src/auth-action/run.mjs record-production   record the corpus twice against the Identity
//                                                    Platform sandbox and update
//                                                    auth-action-production.json
//   node src/auth-action/run.mjs rebuild-fixture <run dir>
//   node src/auth-action/run.mjs check               run the corpus against fireemu and compare
//                                                    with the saved production rows
//   node src/auth-action/run.mjs export-comparison <out.json>
//
...
// FIREEMU_SANDBOX_LEDGER and FIREEMU_AUTH_ACTION_PRIVATE_DIR. AUTH_ACTION_PROGRAMS selects
// programs by id prefix (AUTH_ACTION_PROGRAMS_EXACT=1: by exact id).
//
// Action codes are generated through the Admin sendOobCode with returnOobLink, so production
// mails nothing. The email-link programs switch signIn.email.passwordRequired off for their own
// duration; the session restores it and reads it back, and the run reads the whole baseline
// again at its end.
...
const FIXTURE = join(CONFORMANCE_DIR, "auth-action-production.json");
const RUN_DIR = join(CONFORMANCE_DIR, ".runs", "auth-action");
const LOCAL_PORT = 32298;
/** The synthetic project number fireemu is configured with. */
const LOCAL_PROJECT_NUMBER = "123456789012";
const TASK_ID = "AUTH-ACTION-SANDBOX";
const sha256 = (text) => createHash("sha256").update(text).digest("hex");

function selectedPrograms() {
  const prefixes = (process.env.AUTH_ACTION_PROGRAMS ?? "").split(",").filter(Boolean);
  const exact = process.env.AUTH_ACTION_PROGRAMS_EXACT === "1";
  const programs = prefixes.length
    ? PROGRAMS.filter((p) =>
        prefixes.some((prefix) => (exact ? p.id === prefix : p.id.startsWith(prefix))),
      )
    : PROGRAMS;
  if (programs.length === 0) throw new Error("no program matches AUTH_ACTION_PROGRAMS");
  return programs;
}
...
async function recordOnce(programs, run, web) {
  ...
  // The email-link switch and the rest of the baseline must read back as they started.
...
async function recordProduction() {
  ...
  const privateRoot = process.env.FIREEMU_AUTH_ACTION_PRIVATE_DIR;
  if (!ledger || !privateRoot) {
    throw new Error("FIREEMU_SANDBOX_LEDGER and FIREEMU_AUTH_ACTION_PRIVATE_DIR are required");
  }
  ...
  const runDir = join(privateRoot, `auth-action-production-${meta.startedAt.replaceAll(":", "")}`);
...
async function sessionLocal() {
  const programs = JSON.parse(await readFile(process.env.AUTH_ACTION_IN, "utf8"));
  const ctx = createContext({
    run: process.env.AUTH_ACTION_RUN,
    project: SANDBOX_PROJECT,
    target: {
      kind: "local",
      origin: process.env.AUTH_ACTION_ORIGIN,
...
async function runLocal(programs) {
  ...
  const child = spawn(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        AUTH_ACTION_IN: inPath,
        AUTH_ACTION_OUT: outPath,
        AUTH_ACTION_RUN: String(Date.now()),
        AUTH_ACTION_ORIGIN: `http://127.0.0.1:${LOCAL_PORT}`,
      },
    },
  );
...
async function exportComparison(out) {
  ...
  const evidence = {
    kind: "auth-action-comparison-v1",
