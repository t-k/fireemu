# Held MFA pending credential across provider-unlink: revision 1 (history)

Status: superseded historical record, not approved as current. This revision-1 observation is retained as history; the full receipt, comparison and approvals live in git at commit `8d3a5ca7` under `spec/compatibility/evidence/auth-pending-trigger-provider-unlink/`. The current record is revision 2 (`auth-pending-trigger-provider-unlink.md`), which records each token field of the transition response separately.
