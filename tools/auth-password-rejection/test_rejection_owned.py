...
@pytest.mark.skipif(
    os.getenv("AUTH_PASSWORD_REJECTION_LIVE_LOCAL") != "1",
    reason="owned process opt-in",
)
def test_rejection_owned_lifecycle(tmp_path):
    path = Path(__file__).with_name("rejection_owned.py")
    ...
    from rejection_contract import CASES, complete, validate_case
    from rejection_recorder import recovery_identity
