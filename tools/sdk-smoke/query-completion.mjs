...
try {
  ...
  for (const size of [0, 1, 31, 32, 33, 63, 64, 65, 200]) {
    ...
    assert.deepEqual(
      snapshot.docs.map((doc) => doc.data().index),
      Array.from({ length: size }, (_, i) => i),
    );
  }
  assert.deepEqual(
    (await collection.get()).docs.map((doc) => doc.id),
    ids,
  );
  for (const limit of [1, 31, 32, 33, 64, 200, 201]) {
    assert.deepEqual(
      (await collection.limit(limit).get()).docs.map((doc) => doc.id),
      ids.slice(0, limit),
    );
  }
  for (const offset of [0, 1, 31, 32, 201]) {
    assert.deepEqual(
      (await collection.offset(offset).get()).docs.map((doc) => doc.id),
      ids.slice(offset),
    );
  }
  assert.deepEqual(
    (await collection.orderBy("index", "desc").get()).docs.map((doc) => doc.id),
    ids.toReversed(),
  );
  assert.deepEqual(
    (
  ...
  await db.runTransaction(
    async (transaction) => {
      assert.deepEqual(
        (await transaction.get(collection)).docs.map((doc) => doc.id),
        ids,
      );
    },
    { readOnly: true },
  ...
  console.log(
    JSON.stringify({ ok: true, project: app.options.projectId, documents: 200, boundaryCases: 9 }),
  );
} finally {
  try {
    await deleteFixtures(refs);
  } finally {
    try {
      await db.terminate();
    } finally {
