...
use ftd_core_auth::claims::{ClaimValue, CustomClaims, CustomClaimsError};
use ftd_core_auth::mfa::{MfaError, TotpPolicy};
use ftd_core_auth::store::{AuthStore, NewUser, SecondFactorAssertion};
...
fn secrets_are_redacted_in_debug_output() {
    ...
    let m = s.start_totp_enrollment(&uid, t0()).unwrap();
    let debug = format!("{m:?}");
    let base32 = ftd_core_auth::base32::encode(m.secret_for_test());
    assert!(
