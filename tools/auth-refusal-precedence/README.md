...
| `invalid-token-admin-field-update` | diagnostic | Client `accounts:update` with A's baseline ID token whose signature has one character changed inside the signature, `emailVerified: false` (administrator-only) and a sentinel `displayName`. Sent before any disable so only an invalid signature and a privileged field overlap. A privileged readback of A follows; `invalidTokenStateUnchanged` projects whether the verified flag and the display name are as before. |
