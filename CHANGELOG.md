...
- Strict profile: Storage JSON API list returns 400 for unsupported filters instead of silently ignoring `matchGlob`, `startOffset`, `endOffset`, or `includeTrailingDelimiter=true`. Because object versioning is unavailable, `versions=true` returns the same current generations as `versions=false`. An unsatisfiable media range returns 416 with `Content-Range: bytes */N`; a nonzero suffix on an empty object returns its empty body. The emulator profile keeps the official emulator's responses.
