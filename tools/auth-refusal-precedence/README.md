...
Both accounts are created with private recovery journals, verified UIDs, a verified email and an administrative phone factor enrollment, exactly as in the pending-revocation setup that production accepted. Before each creation the recorder checks that the email is unknown and that the test phone number is not any account's primary number (a factor enrollment is not visible to that lookup; a leftover from an earlier interrupted run is reconciled with `--recover`, not detected here). The rows, in order:
