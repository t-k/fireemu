...
const generateAdmin = program("auth-action/generate/admin", [
  ...
  signIn("sign-in-a", "a"),
  oob("verify-link-by-token", "VERIFY_EMAIL", { idToken: from("sign-in-a:idToken") }),
...
const passwordReset = program("auth-action/password-reset", [
  ...
  reset("reset-consume", "reset-link-2"),
  ...
  adminCreate("create-h", "h"),
  verifyLink("verify-link-h", "h"),
  reset("reset-with-verify-code", "verify-link-h"),
  check("check-verify-code-after-reset-attempt", "verify-link-h"),
  signIn("sign-in-h-original-password", "h"),
]);
...
const verifyEmail = program("auth-action/verify-email", [
  ...
  verifyLink("verify-link-1", "a"),
  verifyLink("verify-link-2", "a"),
  check("check-1", "verify-link-1"),
  check("check-2", "verify-link-2"),
  apply("apply-2", "verify-link-2"),
  apply("apply-2-again", "verify-link-2"),
  apply("apply-1", "verify-link-1"),
  lookupWith("lookup-token-after-verify", "sign-in-a"),
  refreshWith("refresh-after-verify", "sign-in-a", sameSession("id_token", "sign-in-a:idToken")),
  client("apply-unknown-code", "update", { oobCode: "not-a-real-code" }),
  client("apply-truncated-code", "update", { oobCode: chop("verify-link-1:oobCode", 1) }),
  // Already verified.
  verifyLink("verify-link-verified", "a"),
  apply("apply-verified", "verify-link-verified"),
  // The address changed after the code was issued.
  adminCreate("create-b", "b"),
  verifyLink("verify-link-b", "b"),
  adminUpdate("admin-email-change-b", "b", { email: "EMAIL(b-moved)" }),
  check("check-b-after-email-change", "verify-link-b"),
  apply("apply-b-after-email-change", "verify-link-b"),
  adminLookup("admin-lookup-b", "b"),
  // Disabled.
  adminCreate("create-c", "c"),
  verifyLink("verify-link-c", "c"),
  adminUpdate("admin-disable-c", "c", { disableUser: true }),
  apply("apply-c-disabled", "verify-link-c"),
  adminLookup("admin-lookup-c", "c"),
  // Deleted.
  adminCreate("create-e", "e"),
  verifyLink("verify-link-e", "e"),
  adminDelete("admin-delete-e", "e"),
  check("check-e-deleted", "verify-link-e"),
  apply("apply-e-deleted", "verify-link-e"),
  // A reset code applied as a verification.
  adminCreate("create-f", "f"),
  resetLink("reset-link-f", "f"),
  apply("apply-reset-code", "reset-link-f"),
  check("check-reset-code-after-apply-attempt", "reset-link-f"),
...
const changeEmail = program("auth-action/change-email", [
  ...
  adminCreate("create-h", "h"),
  verifyLink("verify-link-h", "h"),
  changeLink("change-link-h", "h", "h-new"),
  apply("apply-change-h", "change-link-h"),
  apply("apply-old-verify-h", "verify-link-h"),
  adminLookup("admin-lookup-h", "h"),
]);
...
const legacyToken = program(
  ...
  [
    ...
    legacySignIn("legacy-sign-in-a", "a"),
    oob("verify-link-by-legacy-token", "VERIFY_EMAIL", {
      idToken: from("legacy-sign-in-a:idToken"),
    }),
    ...
    adminLookup("admin-lookup-a", "a"),
    adminCreate("create-r", "r"),
    legacySignIn("legacy-sign-in-r", "r"),
    signInLink("link-legacy", "r-legacy"),
    emailLinkSignIn("sign-in-link-legacy-token", "link-legacy", "r-legacy", {
      idToken: from("legacy-sign-in-r:idToken"),
    }),
    adminLookup("admin-lookup-r", "r"),
    adminCall("admin-lookup-r-legacy", "lookup", { email: ["EMAIL(r-legacy)"] }),
  ],
  EMAIL_LINK_ON,
);
...
const ownership = program("auth-action/ownership", [
  ...
  apply("apply-a-code-with-b-token", "verify-link-a", { idToken: from("sign-in-b:idToken") }),
  adminCall("admin-lookup-both-after-verify", "lookup", { localId: ["UID(a)", "UID(b)"] }),
  resetLink("reset-link-a", "a"),
  client("check-a-code-naming-b", "resetPassword", {
    oobCode: from("reset-link-a:oobCode"),
    email: "EMAIL(b)",
  }),
  reset("reset-a-code-naming-b", "reset-link-a", "password456", { email: "EMAIL(b)" }),
  signIn("sign-in-a-new-password", "a", "password456"),
  signIn("sign-in-b-original-password", "b"),
  changeLink("change-link-a", "a", "a-new"),
  apply("apply-a-change-with-b-token", "change-link-a", { idToken: from("sign-in-b:idToken") }),
  adminCall("admin-lookup-both-after-change", "lookup", { localId: ["UID(a)", "UID(b)"] }),
  verifyLink("verify-link-b", "b"),
  apply("apply-b-code-with-garbage-token", "verify-link-b", { idToken: "not-a-token" }),
  adminLookup("admin-lookup-b", "b"),
]);
