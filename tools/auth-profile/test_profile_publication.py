...
def publisher():
    path = Path(__file__).parents[1] / "publish-auth-profile.py"
    assert path.exists(), "Profile publication gate is required"
    spec = importlib.util.spec_from_file_location("profile_publication", path)
...
def test_publisher_checks_photo_state_and_rejects_secrets():
    p = publisher()
    row = {
        "id": "set-photo",
        "httpStatus": 200,
    ...
    p.validate_case(row, "set-photo")
    for change in [{"photoState": "second"}, {"idToken": "SECRET"}, {"passed": False}]:
        with pytest.raises(ValueError):
            p.validate_case({**row, **change}, "set-photo")
...
def synthetic_receipt(p):
    ...
    from profile_contract import CHECKS, PHOTO_EXPECTED
    ...
    for target in ("local", "production"):
        ...
        for name in p.CASES:
            ...
            if name in PHOTO_EXPECTED:
                row["photoState"] = PHOTO_EXPECTED[name]
            if name == "signup":
...
def test_receipt_mutations_cannot_bypass_provenance_or_lifecycle_gates():
    ...
    mutations = [
        ...
        (("production", "cases", 2, "photoState"), "SECRET"),
    ]
...
def test_well_formed_mismatch_stays_visible_and_unapproved():
    ...
    row.update(photoState="other", passed=False)
    row["checks"]["photoMatches"] = False
    page = p.render(value)
    assert "| set-photo | Matched | Mismatch |" in page
    assert "candidate, not approved" in page


def test_committed_profile_receipt_and_page_match_without_promoting_old_approval():
    p = publisher()
