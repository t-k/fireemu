...
Behavior below was measured against a real Identity Platform project on 2026-09-24 (AUTH-CREDENTIAL and AUTH-ACTION) and 2026-09-25 (AUTH-CONFIG-SDK). Each item names the profiles it affects; "unlike the official emulator" marks where the emulator profile now differs from the Firebase Emulator Suite.
...
- `GET v2/recaptchaConfig` (the client reCAPTCHA config the Web SDK reads) answers the project's enforcement states per client type and version, with production's `MISSING_CLIENT_TYPE`, `MISSING_RECAPTCHA_VERSION` and invalid client type refusals, in both profiles (the official emulator answers 501). It carries no reCAPTCHA key.
...
- Both profiles: the Admin config PATCH reads its body as production's proto3 JSON and refuses what production refuses with production's words (`Invalid value at 'config.…'`, `Invalid JSON payload received. Unknown name …`, `INVALID_CONFIG : …` for the password policy instead of `INVALID_ARGUMENT`). An unknown, read-only or repeated update mask path is ignored, as production ignores it, instead of refused; a path below a scalar or deeper than any config member is ignored too. A masked parent (`signIn`, `emailPrivacyConfig`, `client.permissions`) the body leaves out goes back to its default, as production does (unlike the official emulator, which keeps the old value). A test phone number's code is taken as any text, in the Admin config and in `auth.signIn` at startup, where six digits were required.
- Strict profile: the Admin config refuses the values production refuses (sandbox recording 2026-09-25): a password policy without exactly one version, region codes outside CLDR (`INVALID_REGION_CODE`), email template updates (`EMAIL_TEMPLATE_UPDATE_NOT_ALLOWED`), reCAPTCHA scores off the tenth steps, rules without an action, SMS switches out of line with the phone state (`INVALID_RECAPTCHA_PHONE_AUTH_CONFIGURATION`), reCAPTCHA keys (`INVALID_SITE_KEY`) and malformed authorized domains. The emulator profile keeps taking a policy without versions as the default policy, as the official emulator takes any policy.
- Both profiles: `v2/passwordPolicy` for an unknown tenant is `400 INVALID_TENANT_ID` (v2 shape), as production answers, instead of `404 TENANT_NOT_FOUND`. Strict: `v1/recaptchaParams` names the producer project number, as production does.
...
- Both profiles: an Admin account update may set a password below six characters, as production lets it (the official emulator refuses it with `WEAK_PASSWORD`). An enforced password policy still applies, an empty password is still refused (unobserved), and end users are still refused.
...
- Strict profile: a written SMS region policy (`smsRegionConfig`) refuses a phone sign-in code for a number of a region it does not allow with production's `OPERATION_NOT_ALLOWED : SMS unable to be sent until this region enabled by the app developer.` Identity Platform documents the policy as based on the destination's calling code: a calling code of one region names that region, and a +1 number is refused only when the policy refuses both the United States and its area code's region. A number whose region fireemu cannot name, and the MFA SMS routes, are not refused (only a +1 650 test number was observed). The emulator profile sends the code, as the official emulator does.
...
- Both profiles: a tenant created with a display name of the documented form (4-20 letters, digits and hyphens, beginning with a letter) is named as production names it: the display name, `-` and five characters of `[a-z0-9]`, drawn reproducibly from the creation sequence. A tenant without such a display name keeps the `fireemu-<sequence>` name. The shape rests on one FS-RULES observation (`fsr-tenant`); case handling and the suffix alphabet beyond `[a-z0-9]` are unobserved.
- Both profiles: a malformed JSON body of an Identity Platform v2 route (`v2/…`, `admin/v2/…`) is refused with the v2 API's `400 INVALID_ARGUMENT` (`Invalid JSON payload received.`), as production refuses it; production's parser diagnostic after that sentence is not reproduced.
