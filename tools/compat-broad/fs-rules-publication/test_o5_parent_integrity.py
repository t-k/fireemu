...
def successful(nonce):
    return {
        "contract": "fs-rules-user-token-local-shadow-run-v1", "status": "LOCAL_SHADOW_ONLY",
        "productionExecuted": False, "productionReady": False, "completed": True,
        "nonce": nonce, "childPid": 111, "recordingComplete": True,
        "stateValidation": True, "resourceCleanupComplete": True, "tenantDeleted": True,
        "firestoreOrigin": "http://127.0.0.1:18081", "authOrigin": "http://127.0.0.1:18082",
    }


def parent_fixture(tmp_path, monkeypatch, change=lambda value: value, *, code=0, stopped=True):
    ...
    def launch(argv, **kwargs):
        ...
        if value is not None:
            ...
            if value == "symlink": path.symlink_to(binary)
            elif value == "fifo": os.mkfifo(path)
            elif isinstance(value, str): path.write_text(value)
            else: path.write_text(json.dumps(value))
        return Child()

    monkeypatch.setattr(module.subprocess, "Popen", launch)
    monkeypatch.setattr(module, "_stop_owned_process", lambda _child: calls.append(("stop",)) or stopped)
    monkeypatch.setattr(module, "_socket_closed", lambda origin: origin in {
        "http://127.0.0.1:18081", "http://127.0.0.1:18082",
    })
    return output, calls


@pytest.mark.parametrize("field", ["completed", "recordingComplete", "stateValidation", "resourceCleanupComplete", "tenantDeleted"])
@pytest.mark.parametrize("value", [False, 1, None, "true"])
def test_zero_child_exit_does_not_replace_typed_acceptance(tmp_path, monkeypatch, field, value):
...
@pytest.mark.parametrize("change", [
    lambda v: None, lambda v: "{invalid", lambda v: "[]", lambda v: "symlink", lambda v: "fifo",
    lambda v: {**v, "nonce": "other"}, lambda v: {**v, "contract": "other"},
    lambda v: {**v, "productionExecuted": True}, lambda v: {**v, "productionReady": 0},
    lambda v: {**v, "childPid": True}, lambda v: {**v, "failure": "failure"},
...
def test_exit_timeout_or_unconfirmed_stop_keeps_failure(tmp_path, monkeypatch, code, stopped):
...
def test_existing_output_refused_before_build(tmp_path, monkeypatch, kind):
    ...
    elif kind == "file": output.write_text("original")
    else:
        target = tmp_path / "target"
        if kind == "symlink": target.mkdir()
        output.symlink_to(target)
    monkeypatch.setattr(module, "build", lambda: pytest.fail("build must not start"))
    with pytest.raises(module.Refused): module.run_parent(output)


def test_build_failure_is_recorded_without_fabricated_child_success(tmp_path, monkeypatch):
    def fail(): raise FileNotFoundError("private-path")
...
def test_publication_failure_cannot_return_zero(tmp_path, monkeypatch):
    ...
    def publish(path, value):
        if path.name == "parent-result.json": raise OSError("disk-full")
        return real(path, value)
    monkeypatch.setattr(module, "_publish_new", publish)
    with pytest.raises(OSError): module.run_parent(output)


@pytest.mark.parametrize("raw", ['{"completed": false, "completed": true}', '{"a":NaN}', '{"a":Infinity}'])
def test_child_json_rejects_duplicate_keys_and_nonfinite_values(tmp_path, raw):
    file = tmp_path / "child"
    file.write_text(raw)
    with pytest.raises(module.Refused): module._read_child_receipt(file)


def test_publisher_never_overwrites_and_removes_temporary_file(tmp_path):
    file = tmp_path / "receipt.json"
    file.write_text("original")
    with pytest.raises(FileExistsError): module._publish_new(file, {"ok": True})
    assert file.read_text() == "original"
    assert list(tmp_path.glob(".receipt-*")) == []


@pytest.mark.parametrize("err,expected", [(errno.ECONNREFUSED, True), (errno.ETIMEDOUT, False),
    (errno.EHOSTUNREACH, False), (errno.EACCES, False)])
def test_only_connection_refusal_proves_closed(monkeypatch, err, expected):
    def connect(*_a, **_k): raise OSError(err, "test")
    monkeypatch.setattr(module.socket, "create_connection", connect)
    assert module._socket_closed("http://127.0.0.1:4321") is expected


@pytest.mark.parametrize("origin", ["http://remote.invalid:9", "http://127.0.0.1", "http://u@127.0.0.1:9", "http://127.0.0.1:9/path", "http://127.0.0.1:9#x"])
def test_unknown_origin_does_not_become_vacuous_closure(monkeypatch, origin):
    monkeypatch.setattr(module.socket, "create_connection", lambda *_a, **_k: pytest.fail("network reached"))
...
def test_foreign_process_group_is_not_signalled(monkeypatch):
    ...
    monkeypatch.setattr(module.os, "killpg", lambda *_: pytest.fail("foreign group signalled"))
    assert module._stop_owned_process(Child()) is False
