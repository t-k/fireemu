...
def cases(collection: str, extended: bool = False, details: bool = False) -> list[dict]:
    rows = []
    for label, field in [
        ("omitted", None),
        ("name-asc", "__name__"),
    ...
    if details:
        reference = {"referenceValue": f"{DATABASE}/documents/{collection}/A"}
        number = {"integerValue": "10"}
        x = {"field": {"fieldPath": "x"}, "direction": "ASCENDING"}
        name = {"field": {"fieldPath": "__name__"}, "direction": "ASCENDING"}
        inequality = {
            "fieldFilter": {
                "field": {"fieldPath": "y"},
                "op": "GREATER_THAN",
        ...
        for label, clauses, count_only in [
            (
                "cursor-x-two",
                {
                    "orderBy": [x],
            ...
            ("inequality-y-implicit", {"where": inequality}, False),
            ("inequality-y-explicit-x", {"where": inequality, "orderBy": [x]}, False),
            (
                "count-cursor-reference",
                {"startAt": {"values": [reference], "before": False}},
                True,
            ),
            (
            ...
            row = deepcopy(rows[0])
            row["id"] = label
            aggregate = row["body"]["structuredAggregationQuery"]
            aggregate["structuredQuery"].update(deepcopy(clauses))
            if count_only:
                aggregate["aggregations"] = [{"alias": "count", "count": {}}]
            rows.append(row)
    return rows
...
def observe(
    ...
    require(
        collection is None or target == "local",
        "production namespace must be generated by the recorder",
    )
    collection = collection or "compat_" + uuid.uuid4().hex
    ...
    report = {
        ...
        "caseSha256": fingerprint(cases("COLLECTION", extended, details)),
    ...
    try:
        ...
        for case in cases(collection, extended, details):
            status, raw = request(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--details",
        action="store_true",
        help="Add cursor arity and inequality-order diagnostics without creating an index",
    )
    parser.add_argument(
        "--collection",
        help="Local-only private UUID collection matching the owned launch index configuration",
    )
    args = parser.parse_args()
    result = observe(
        args.target, args.output, args.extended, args.details, args.collection
    )
    print(
        json.dumps(
            {
