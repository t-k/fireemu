...
def _auth_recovery_fixture_with_second_creation(tmp_path, *, unknown_kind=False, consumed_missing=False):
    fixture = _auth_recovery_fixture(tmp_path)
    ledger, parent, child, envelope, child_plan, bindings, evidence, resource, child_path = fixture
    parent_path = Path(parent["ledgerPath"]).parent / "auth-parent-gate"
    parent_gate = Gate(parent_path, "auth-credential").snapshot()
    ...
    operation = {
        ...
        "kind": "unrecognized-creating-operation" if unknown_kind else "sign-up",
    ...
    consumed_count = 2 if unknown_kind or consumed_missing else 1
    ...
    parent_gate["jobs"]["auth-credential"]["scheduleDone"] = 2 if unknown_kind else 1
    if unknown_kind:
