...
export function programFixture() {
  ...
  const update = (path, fields, extra = {}) => ({ update: { name: `${names}/${path}`, fields }, ...extra });
  return { id: 'writes/batch-write', area: 'writes',
    seed: [{ path: `${docs}/bw/existing`, fields: { a: int(1) } }],
    steps: [
      { id: 'non-atomic-batch', method: 'POST', path: `${docs}:batchWrite`, body: {
        writes: [update('bw/one', { a: int(1) }),
          ...
          { transform: { document: `${names}/bw/one`, fieldTransforms: [{ fieldPath: 'n', increment: int(5) }] } },
        ] } },
      ...
      { id: 'batch-with-transaction-is-refused', method: 'POST', path: `${docs}:batchWrite`, body: {
        writes: [update('bw/two', { a: int(1) })], transaction: 'AA==' } },
    ] };
}
export function productionRows() {
  return {
    'non-atomic-batch': { status: 400, code: 'INVALID_ARGUMENT',
      message: 'the same document cannot be written more than once in a single request' },
    'one-was-written': { status: 404, code: 'NOT_FOUND',
      message: 'Document "projects/demo-firestore-probe/databases/(default)/documents/bw/one" not found.' },
    'existing-was-deleted': { status: 200, code: 'OK', body: {
      name: 'projects/demo-firestore-probe/databases/(default)/documents/bw/existing',
      fields: { a: { integerValue: '1' } }, createTime: '<now>', updateTime: '<now>',
    } },
    'empty-batch': { status: 200, code: 'OK', body: {} },
    'batch-with-transaction-is-refused': { status: 400, code: 'INVALID_ARGUMENT',
...
export function matrixFixture() {
  return { version: 1, evidence: { verified: true, validation: [], observations: { production: {
    ...
    observation: { side: 'production', mode: 'live', database: {
      type: 'FIRESTORE_NATIVE', databaseEdition: 'STANDARD' } },
    inputs: { corpusDigest: CASE.corpusDigest },
  } } }, programs: [{ id: CASE.programId, area: 'writes', steps:
    Object.fromEntries(Object.entries(productionRows()).map(([id, row]) => [id, { production: row,
      // Deliberately conflicting auxiliary values; the new path must ignore them.
      emulator: { status: 500, code: 'INTERNAL' }, fireemu: { missing: true }, status: 'debt' }])) }] };
}
