...
def run(output):
    ...
    process = None
    with tempfile.TemporaryDirectory(prefix="fireemu-password-owned-") as temporary:
        private = Path(temporary)
        artifact = private / "fireemu"
        verified_fd = build.get("_launchCopyFd")
        owns_fd = verified_fd is None
        if owns_fd:
