...
const requiredRecipes = new Map([
  ...
  [
    "FS-LIMIT-FIELD-PATH-BYTES",
    new Set([
      "writes/limits/field-path-bytes",
...
test("FS-DATA-WRITE closure inventory cannot silently omit a declared condition", () => {
  ...
  for (const condition of closure.conditions) {
    ...
    assert.ok(
      ["PENDING_CORPUS", "SAVED_REFERENCE_PENDING_FINAL", "PRODUCTION_RECORDED", "MISMATCH", "VERIFIED", "PENDING_REVIEW"].includes(condition.status),
  ...
  for (const [conditionId, recipes] of requiredRecipes) {
    ...
    assert.deepEqual(new Set(condition.recipeIds), recipes, `${conditionId}: incomplete recipe mapping`);
  }
