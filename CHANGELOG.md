...
- Strict profile: a factor withdrawal ends every session before it, as a phone enrollment does (a TOTP enrollment does not), and answers without `expiresIn` with a session that keeps the asking session's second factor unless that factor was withdrawn; a missing token is `INVALID_ID_TOKEN` and a missing factor id `MFA_ENROLLMENT_NOT_FOUND` (sandbox recording 2026-09-24, `auth-mfa/totp/withdraw`, `sms`).
