...
CURRENT_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v8.json"
CURRENT_BINDING = PACKAGE_DIR / "fs-write-txn-precedence-01-v8-binding.json"
CURRENT_SHADOW = PACKAGE_DIR / "fs-write-txn-precedence-01-v8-local-shadow.json"
PREVIOUS_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v7.json"

EXPECTED_HEAD = "27c07e041c3ee5bf21eff9beff2cfc62131c6ab3"
EXPECTED_ARTIFACT = "ff9b92d745fb919bd2af9478d6b09a886335919ad64e75daf7021171ffaef368"
...
def test_v8_is_a_new_immutable_package_bound_to_current_head() -> None:
    ...
    assert manifest["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V8"
    assert binding["campaignId"] == manifest["campaignId"]
    assert shadow["campaignId"] == manifest["campaignId"]
    assert manifest["preparation"]["supersedes"] == "FS-WRITE-TXN-PRECEDENCE-01-V7"
    assert previous["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V7"
...
def test_v8_artifact_and_manifest_bindings_are_consistent() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
    ...
    assert manifest["sourceBinding"]["artifactBuild"]["relativePath"] == "target/o3-fs-write-v8/release/fireemu"
    assert binding["artifactBuild"]["relativePath"] == "target/o3-fs-write-v8/release/fireemu"
    assert shadow["artifactBuild"]["relativePath"] == "target/o3-fs-write-v8/release/fireemu"
    ...
    assert binding["manifest"] == {
        "path": "spec/compatibility/broad-runs/fs-write-txn-precedence-01-v8.json",
...
def test_v8_remains_blocked_without_owner_or_stream_comparator() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
