"""Build and own the strict Auth artifact with the local disabling fixture; run the same
readback corpus against it. Never attaches to an external daemon."""
...
FIXTURE = (
    Path(__file__).resolve().parent.parent / "auth-blocking-disable" / "function-local"
)
...
def run(output):
    ...
    process = None
    with tempfile.TemporaryDirectory(prefix="fireemu-readback-owned-") as temporary:
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise SystemExit("Readback owned observation did not complete") from None
