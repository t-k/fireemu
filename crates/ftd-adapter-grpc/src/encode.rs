...
use ftd_core_firestore::value::{Timestamp, Value};
...
pub fn encode_value(v: &Value) -> pb::Value {
    ...
    let value_type = match v {
        ...
        Value::GeoPoint(g) => V::GeoPointValue(ftd_proto_firestore::google::r#type::LatLng {
            latitude: g.latitude(),
...
/// Firestore core error → gRPC status (wire mapping revision in `ftd-core-limits`).
#[must_use]
pub fn status_from_error(e: &FirestoreError) -> tonic::Status {
    ...
    if let FirestoreError::ResourceExhausted(v) = e {
        if let Ok(value) = v.limit_id.parse() {
            status.metadata_mut().insert("ftd-limit-id", value);
        }
    }
    status
}
