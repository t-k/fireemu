...
2. The free-tier allowance applies to the default database alone. A second database leaves the free tier while it exists, which is why it holds no data and is deleted in the same run.
...
Cleanup runs in a fixed order: revert the field configurations, verify each against the baseline captured before its patch, delete the created database, verify its absence, then write the final ledger. A run with any unrecovered resource exits non-zero and is not a valid observation, even if every case otherwise completed.
...
`tools/compat-broad/fs-config-lifecycle/rehearsal.py` replays that path offline under six injected failures: none, an unexpected pre-existing database, a refused create, a poll deadline, an interrupt after the create, and a refused revert. It issues no request. Only the clean outcome exits zero, an unrecovered resource is always reported as such, and the interrupted case still deletes the database it created.

## Comparator contract

`tools/compat-broad/fs-config-lifecycle/comparator.py` accepts preparation receipts only and always returns `PREPARATION_ONLY` with no rows. It rejects observation-shaped fields, a forged source binding, a receipt claiming execution, a receipt bound to a different manifest, and a drifted manifest.
