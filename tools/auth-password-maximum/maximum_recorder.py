...
def password_policy(status, value, passwords):
    ...
    require(
        isinstance(passwords[0], str)
        and re.fullmatch(r"Aa9![A-Za-z0-9_-]{43}", passwords[0])
    )
    require(
        isinstance(passwords[1], str)
        and re.fullmatch(r"[A-Za-z0-9_-]{4096}", passwords[1])
    )
    return PASSWORD_POLICY.copy()


def boundary_passwords():
    maximum = secrets.token_urlsafe(3072)
    tail = maximum[:-1] + ("A" if maximum[-1] != "A" else "B")
    return maximum, tail, maximum[:-1], maximum + "A"


def input_shape(original, maximum, tail, prefix, oversize):
    require(
    ...
    require(
        all(
            ...
            for v in (maximum, tail, prefix, oversize)
        )
    )
    require(
        len(maximum) == 4096
        and len(tail) == 4096
        and tail[:-1] == maximum[:-1]
        and tail[-1] != maximum[-1]
    )
    require(
        prefix == maximum[:-1] and oversize[:-1] == maximum and len(oversize) == 4097
    )
    return {
        "originalLength": 47,
        "maximumLength": 4096,
...
def observe(output, origin=None):
    ...
    replacement, tail, prefix, oversize = boundary_passwords()
    report["inputShape"] = input_shape(password, replacement, tail, prefix, oversize)
    uid = None
    attempted = False
    ...
    try:
        ...
        observe_tokens("maximum-password-change", status, changed)
        observe_state("maximum-token-lookup", changed["idToken"], initial)

        def refused_signin(name, candidate):
            status, value = client(
                "signInWithPassword",
                {"email": email, "password": candidate, "returnSecureToken": True},
            )
            report["cases"].append(rejection(name, status, value))

        refused_signin("old-password-rejected", password)
        status, maximum_signin = client(
            "signInWithPassword",
            {"email": email, "password": replacement, "returnSecureToken": True},
        )
        observe_tokens("maximum-password-signin", status, maximum_signin)
        observe_state("maximum-password-lookup", maximum_signin["idToken"], initial)

        def refresh_maximum(name, lookup_name):
            ...
            observe_tokens(name, status, value, refresh=True)
            observe_state(lookup_name, value["id_token"], initial)

        refresh_maximum("maximum-token-refresh", "maximum-refreshed-lookup")
        refused_signin("tail-password-rejected", tail)
        observe_state("state-after-tail", maximum_signin["idToken"], initial)
        refused_signin("prefix-password-rejected", prefix)
        observe_state("state-after-prefix", maximum_signin["idToken"], initial)
        status, refused = client(
            "update",
            {
                "idToken": maximum_signin["idToken"],
                "password": oversize,
                "returnSecureToken": True,
            },
        )
        report["cases"].append(rejection("oversize-password-rejected", status, refused))
        observe_state("state-after-oversize", maximum_signin["idToken"], initial)
        status, preserved = client(
            "signInWithPassword",
            {"email": email, "password": replacement, "returnSecureToken": True},
        )
        observe_tokens("preserved-maximum-signin", status, preserved)
        observe_state("preserved-maximum-lookup", preserved["idToken"], initial)
        refresh_maximum("preserved-maximum-refresh", "preserved-refreshed-lookup")
        token = preserved["idToken"]
