...
use fireemu_core_auth::base32::{decode, encode};
use fireemu_core_auth::totp::{hotp, time_step, totp_at, TotpParams};
...
fn time_step_boundaries_on_the_virtual_clock() {
    ...
    assert_eq!(
        time_step(
            &p,
            t.checked_add(fireemu_core_types::time::LogicalDuration::from_seconds(29))
                .unwrap()
        ),
        59_600_162
    );
    assert_eq!(
        time_step(
            &p,
            t.checked_add(fireemu_core_types::time::LogicalDuration::from_seconds(30))
                .unwrap()
        ),
        59_600_163
    );
