...
async function server(handler, fn) {
  ...
  try { return await fn(`http://127.0.0.1:${s.address().port}`); }
  finally { s.closeAllConnections(); await new Promise(r => s.close(r)); }
}
const child = code => runProcess(process.execPath, ['--input-type=module', '-e', code],
  { env: cleanEnvironment('/tmp'), timeoutMs: 4000 });
for (const host of ['localhost:8011', 'firestore.googleapis.com:443', '127.0.0.1:0',
  '127.0.0.1:65536', '[::1]:8080', '127.0.0.1:8080/path']) test(`reject endpoint ${host}`, () =>
  assert.throws(() => localOrigin(host)));
test('only exact numeric loopback origin is accepted', () => {
  ...
  for (const value of ['http://127.0.0.1:8081/x', 'https://127.0.0.1:8080/x',
    'http://u:p@127.0.0.1:8080/x', 'http://localhost:8080/x', origin + '/x#secret'])
    assert.throws(() => assertUrl(value, origin));
});
test('real fetch succeeds on the owned loopback origin', async () => {
  await server((q, r) => { r.setHeader('content-type', 'application/json'); r.end('{"ok":true}'); }, async origin => {
    ...
    assert.equal(result.code, 0, result.log.toString()); assert.match(result.log.toString(), /"ok":true/);
  });
});
test('redirect cannot reach a second loopback server', async () => {
  let reached = 0;
  await server((q,r) => { reached++; r.end('bad'); }, async target => {
    await server((q,r) => { r.writeHead(302, { location: target }); r.end(); }, async origin => {
      const result = await child(`import {installNetworkGuard} from ${JSON.stringify(guardUrl)};
        installNetworkGuard(${JSON.stringify(origin)});
        try { await fetch(${JSON.stringify(origin)}); process.exitCode=3; }
        catch { console.log('redirect-refused'); }`);
      assert.equal(result.code, 0, result.log.toString()); assert.equal(reached, 0);
    });
  });
});
...
test('offline comparison denies network, DNS and credential commands', async () => {
  ...
  assert.equal(result.code, 0, result.log.toString()); assert.equal(result.log.toString().trim(), '7');
});
