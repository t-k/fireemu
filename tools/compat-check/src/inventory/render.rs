...
fn features(inv: &Inventory, path: &str, title: &str) -> String {
    ...
    for f in features {
        ...
        if text(f, "id") == "AUTH-USERS" {
            out.push_str("[No-password-change session continuity control](auth-session-continuity.md). Independent, unapproved candidate observations of fixed A/B and reference-exchange credentials at finite 0/10/30-second targets. Continuity requires accepted primary and derived-token requests, not matching rejections. This does not establish permanent validity, whole-session lineage, SDK behavior or actual elapsed expiry.\n\n");
