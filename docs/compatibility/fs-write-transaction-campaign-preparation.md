...
The current package prepares the next bounded Oracle campaign for `FS-DATA-WRITE` and `FS-TRANSACTION` at feature head `97ef1d3a8fb52938bbe26b1da9c85b9c5b430dd3`. It is preparation and local-shadow evidence only. It does not contain a production receipt, does not authorize a Cloud request and does not promote either parent group. The earlier `FS-WRITE-TXN-PRECEDENCE-01` package remains preserved as an immutable historical binding to the earlier source and artifact.

The current machine-readable package is [`fs-write-txn-precedence-01-v6.json`](../../spec/compatibility/broad-runs/fs-write-txn-precedence-01-v6.json). Its companion binding is [`fs-write-txn-precedence-01-v6-binding.json`](../../spec/compatibility/broad-runs/fs-write-txn-precedence-01-v6-binding.json), and the local-only shadow plan is [`fs-write-txn-precedence-01-v6-local-shadow.json`](../../spec/compatibility/broad-runs/fs-write-txn-precedence-01-v6-local-shadow.json). The historical package and v2/v3/v4/v5 packages remain preserved as immutable bindings; v6 is the current-head package after the Auth settings integration. The `codeSourceHead` in the package identifies the repository revision used to build the artifact; the package/evidence commit that adds this rebind is tracked separately in Git and is intentionally not embedded in its own digest-bound JSON.

## Current, next and backlog

| Queue position | Campaign | Scope | State |
| --- | --- | --- | --- |
| CURRENT | `FS-WRITE-TXN-PRECEDENCE-01-V6` | A Write stream request contending with an active read-write transaction, followed by owned document-size and nesting controls | `BLOCKED_OWNER` and `BLOCKED_TECHNICAL` |
