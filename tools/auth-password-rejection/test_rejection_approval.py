"""Rejection approval is bound to one immutable receipt and 16 observations."""
...
def tool():
    path = Path(__file__).parents[1] / "auth-password-rejection-approval.py"
    assert path.exists(), "Subject-bound approval overlay required"
    spec = importlib.util.spec_from_file_location("rejection_approval", path)
...
def test_changed_receipt_cannot_reuse_approval(change):
    ...
    if change == "artifact":
        ...
        receipt["local"]["cases"][4]["observedError"] = "INVALID_ID_TOKEN"
    with pytest.raises(ValueError):
