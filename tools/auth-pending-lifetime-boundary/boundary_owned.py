"""Build and own the strict Auth artifact; run the pending-lifetime boundary corpus
(revision 2) against it, aging pendings by advancing the owned instance's virtual clock past
the local pending lifetime so a refusal is observed. Never attaches to an external daemon."""
...
def run(output):
    ...
    with tempfile.TemporaryDirectory(
        prefix="fireemu-pending-lifetime-boundary-owned-"
    ) as temporary:
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise SystemExit(
            "Pending-lifetime boundary owned observation did not complete"
        ) from None
