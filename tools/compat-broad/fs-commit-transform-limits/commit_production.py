...
def _expected(plan: dict[str, Any], operation: dict[str, Any], recovery: bool, index: int) -> dict[str, Any]:
    ...
    *,
    ...
    def dispatch(operation: dict[str, Any], recovery: bool, index: int, *, row_name: str) -> dict[str, Any]:
        ...
        def send():
            ...
            if operation.get("kind") != "cleanup-conditional-delete" and digest(operation) != digest(expected):
                raise ValueError("request differs from frozen compiler plan")
            raw = bridge.send(operation) if operation.get("kind") == "commit-transform" else classify_receipt(transmit(copy.deepcopy(operation)))
            holder["receipt"] = raw
        ...
        try:
            ...
            if isinstance(result, tuple):
                status, body = result
                if status is None:
                    entry.update({"complete": True, "failure": None, "status": None, "body": body, "skipped": body.get("skipped")})
                else:
                    entry.update({"complete": True, "failure": None, "status": status, "body": body})
