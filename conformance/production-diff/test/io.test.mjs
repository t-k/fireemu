...
import { cleanEnvironment, readSource, newPrivateDirectory, publish, runProcess,
  snapshotBinary } from '../io.mjs';
...
test('only an explicit small environment crosses the process boundary', () => {
  ...
  for (const key of ['NODE_OPTIONS', 'HTTPS_PROXY', 'HTTP_PROXY', 'ALL_PROXY', 'GOOGLE_APPLICATION_CREDENTIALS',
    'FIRESTORE_PROBE_TOKEN', 'FIRESTORE_PROBE_TARGET', 'ANTHROPIC_API_KEY', 'GITHUB_TOKEN', 'GIT_CONFIG_COUNT'])
    assert.equal(e[key], undefined);
  assert.equal(e.HOME, '/private/home'); assert.equal(e.GIT_NO_REPLACE_OBJECTS, '1'); assert.equal(e.GIT_OPTIONAL_LOCKS, '0');
});
test('output is outside the repo, private and never reused', () => area(async root => {
...
test('result publication does not overwrite files', () => area(async root => {
  const p = join(root, 'result.json'); await publish(p, 'one');
  await assert.rejects(publish(p, 'two')); assert.equal(await fs.readFile(p, 'utf8'), 'one');
  assert.deepEqual(await fs.readdir(root), ['result.json']);
}));
test('result publication does not follow a symlink', () => area(async root => {
  const actual = join(root, 'keep'); await fs.writeFile(actual, 'keep');
...
test('readSource rejects file and directory symlink traversal', () => area(async root => {
  await fs.mkdir(join(root, 'repo')); await fs.mkdir(join(root, 'external'));
...
for (const p of ['../x', '/etc/passwd', 'a/../x', 'a//x', 'a\\x']) test(`unsafe input path ${p}`, () =>
  assert.rejects(readSource('/tmp', p), /unsafe-source-path/));
test('readSource bounds input size and supports spaces/Japanese', () => area(async root => {
  const dir = join(root, '作業 space'); await fs.mkdir(dir);
...
test('shell scripts cannot be presented as a native fireemu binary', () => area(async root => {
  const f = join(root, 'fake'); await fs.writeFile(f, '#!/bin/sh\nexit 0\n');
...
test('CLI does not provide a production backend or a server endpoint', () => {
  ...
  assert.throws(() => parseArgs(['replay', '--endpoint', 'https://firestore.googleapis.com']), /arguments/);
...
test('exec uses argument arrays, private config and OS-assigned ports', () => {
  ...
  assert.equal(c.command, '/binary'); assert.equal(c.args.at(-1), '/entry with spaces.mjs');
  for (const flag of ['--firestore-port', '--http-port', '--ui-port', '--hub-port'])
    assert.equal(c.args[c.args.indexOf(flag) + 1], '0');
  assert.ok(c.args.includes('/a b/日本語/fireemu.json')); assert.ok(!c.args.includes('sh'));
});
test('bounded process captures an ordinary completion', async () => {
  const p = await runProcess(process.execPath, ['-e', 'console.log("ok")'],
    { env: cleanEnvironment('/tmp'), timeoutMs: 2000 });
  assert.equal(p.code, 0); assert.equal(p.reason, null); assert.equal(p.state, 'stopped');
...
test('process timeout is an error and the owned group is stopped', async () => {
  const p = await runProcess(process.execPath, ['-e', 'setInterval(()=>{},1000)'],
    { env: cleanEnvironment('/tmp'), timeoutMs: 100 });
  assert.equal(p.reason, 'process-timeout'); assert.notEqual(p.code, 0); assert.equal(p.state, 'stopped');
});
test('oversized logs stop the process', async () => {
  const p = await runProcess(process.execPath, ['-e', 'console.log("x".repeat(20000));setInterval(()=>{},1000)'],
    { env: cleanEnvironment('/tmp'), maxLogBytes: 100, timeoutMs: 2000 });
  assert.equal(p.reason, 'process-log-limit'); assert.ok(p.log.length <= 100);
});
test('missing executable is not a pass', async () => {
  const p = await runProcess('/nonexistent-fireemu-pilot-binary', [],
    { env: cleanEnvironment('/tmp'), timeoutMs: 100 });
  assert.equal(p.reason, 'spawn-failed'); assert.notEqual(p.code, 0);
});
