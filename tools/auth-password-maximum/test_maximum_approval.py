"""Minimum approval is bound to one immutable receipt and 21 observations."""
...
def tool():
    path = Path(__file__).parents[1] / "auth-password-maximum-approval.py"
    assert path.exists(), "Subject-bound approval overlay required"
    spec = importlib.util.spec_from_file_location("maximum_approval", path)
...
def test_changed_receipt_cannot_reuse_approval(change):
    ...
    if change == "artifact":
        ...
        receipt["local"]["inputShape"]["maximumLength"] = 7
    with pytest.raises(ValueError):
