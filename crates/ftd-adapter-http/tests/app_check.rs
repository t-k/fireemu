...
use ftd_adapter_http::app_check::{AppCheckState, RawRequest};
use ftd_adapter_http::identity_toolkit::{AuthState, JsonResponse, RequestHeaders};
use ftd_adapter_http::signing::{
...
use ftd_core_app_check::verify::verify_token;
...
fn call_with(
    ...
    ftd_adapter_http::app_check::handle_raw(
        state,
        &RawRequest {
            method,
            path,
...
fn a_registered_debug_token_exchanges_for_a_verifiable_session_token() {
    ...
    assert!(state.signer.kid().starts_with("ftd-app-check-"));
    assert!(token.contains(&ftd_core_app_check::jwt::base64url_encode(
        format!(
...
fn two_daemon_instances_reject_each_others_locally_issued_tokens() {
    ...
    assert_eq!(
        ...
        Err(ftd_core_app_check::verify::AppCheckFailure::UnknownKeyId),
    ...
    assert_eq!(
        ...
        Err(ftd_core_app_check::verify::AppCheckFailure::BadSignature)
    );
}
...
fn a_created_debug_token_returns_its_secret_once_and_then_only_a_digest_prefix() {
    ...
    assert!(ftd_core_app_check::exchange::canonical_debug_token(&secret).is_some());
...
async fn the_exchange_and_the_jwks_are_served_over_a_real_socket_and_never_cached() {
    ...
    let server = tokio::spawn(ftd_adapter_http::server::serve(listener, state));
...
async fn every_app_check_route_is_not_found_while_the_section_is_disabled() {
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let server = tokio::spawn(ftd_adapter_http::server::serve(listener, auth_state(None)));
