...
fn a_future_issued_at_is_rejected_even_with_a_valid_expiry_and_authentication_time() {
    ...
    for ahead in [1, 600, 7200] {
        ...
        let token = encode_unsigned(&claims);
        assert!(decode_unsigned(&token).is_ok(), "this is not a JSON/signature test");
        assert_eq!(verify_id_token(&token, &store, at(NOW)), Err(JwtError::Malformed));
    }
}
...
fn authentication_time_remains_required_and_typed() {
    let (store, claims, _) = fixture();
    for raw in [None, Some("null"), Some("true"), Some("\"1788004860\""), Some("[]")] {
...
fn a_refreshed_token_preserves_the_older_authentication_time() {
    ...
    assert_eq!(decoded.payload.get("auth_time").and_then(JsonValue::as_i64), Some(NOW - 300));
    assert_eq!(decoded.payload.get("iat").and_then(JsonValue::as_i64), Some(NOW));
}
...
fn the_subsecond_before_issuance_is_refused_but_the_issuance_instant_is_valid() {
    ...
    assert_eq!(verify_id_token(&token, &store, before), Err(JwtError::Malformed));
    assert!(verify_id_token(&token, &store, at(NOW)).is_ok());
    assert!(verify_id_token(&token, &store, LogicalInstant::from_nanos(at(NOW).as_nanos() + 1)).is_ok());
}
...
fn expiry_is_still_exclusive_and_has_its_existing_error() {
    ...
    assert_eq!(verify_id_token(&token, &store, at(claims.exp)), Err(JwtError::Expired));
    let missing_iat = with_json_claim(&claims, "iat", None);
    assert_eq!(verify_id_token(&missing_iat, &store, at(claims.exp)), Err(JwtError::Expired));
}
...
fn disabled_and_revoked_accounts_still_refuse_well_timed_tokens() {
    ...
    assert_eq!(verify_id_token(&token, &store, at(NOW)), Err(JwtError::UserDisabled));
    store.user_mut(&uid).unwrap().disabled = false;
    store.revoke_tokens(&uid, at(NOW - 200)).unwrap();
    assert_eq!(verify_id_token(&token, &store, at(NOW)), Err(JwtError::Revoked));
}
...
fn verified_rules_entry_points_apply_both_time_checks() {
    ...
    for key in ["iat", "auth_time"] {
        ...
        assert_eq!(
            verify_rules_token_for_project(&token, &store, at(NOW), TokenAcceptance::Verified, PROJECT),
            Err(JwtError::Malformed)
        );
    }
}
...
fn namespace_checks_are_not_replaced_by_the_time_check() {
    ...
    assert!(matches!(verify_id_token(&encode_unsigned(&claims), &store, at(NOW)), Err(JwtError::WrongAudience { .. })));
    claims.aud = PROJECT.to_owned();
    claims.firebase.tenant = Some("other-tenant".to_owned());
    assert!(matches!(verify_id_token(&encode_unsigned(&claims), &store, at(NOW)), Err(JwtError::WrongTenant { .. })));
    claims.iss = "https://securetoken.google.com/demo-other".to_owned();
    assert!(matches!(verify_id_token(&encode_unsigned(&claims), &store, at(NOW)), Err(JwtError::WrongIssuer { .. })));
}

#[test]
fn verification_does_not_consume_or_modify_the_account_on_a_time_refusal() {
    let (store, claims, uid) = fixture();
    let before = store.id_token_claims(&uid, None, at(NOW)).unwrap().canonical_json();
    ...
    let after = store.id_token_claims(&uid, None, at(NOW)).unwrap().canonical_json();
...
fn very_large_time_claims_do_not_require_duration_arithmetic() {
    ...
    claims.exp = i64::MAX;
    claims.iat = i64::MAX;
    assert_eq!(verify_id_token(&encode_unsigned(&claims), &store, at(NOW)), Err(JwtError::Malformed));
    claims.iat = NOW;
