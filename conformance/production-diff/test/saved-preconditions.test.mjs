...
import { CASE, TRANSFORMS_CASE, PRECONDITIONS_CASE as entry, selectCase } from '../registry.mjs';
...
test('public CLI selects the new case without changing the default',async()=>{
 ...
 assert.deepEqual(JSON.parse(output).cases.map(c=>c.id),[CASE.id,'fs.commit-transform-limits.saved-031c74bfe.v1','fs.g0.saved-68012694.v1',TRANSFORMS_CASE.id,entry.id]);
});
