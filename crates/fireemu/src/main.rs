...
//! ```text
//! fireemu up [options]
//! fireemu exec [options] [--only auth,firestore,storage,functions,appcheck] -- <command...>
//! fireemu doctor
//! fireemu capabilities
//!
//! options: [--config fireemu.json] [--firebase-json firebase.json] [--project <id>]
...
fn usage() -> ExitCode {
    eprintln!("usage: fireemu up {OPTIONS_USAGE}\n       fireemu exec {OPTIONS_USAGE} -- <command...>\n       fireemu doctor\n       fireemu capabilities");
    ExitCode::from(2)
}
...
fn parse_options(args: &[String]) -> Result<(RuntimeConfig, Selection), String> {
    ...
    if let Some(path) = firebase_json {
        ...
        if !ignored.is_empty() {
            eprintln!(
                "note: {} has no equivalent in fireemu and is ignored: {}",
                path.display(),
                ignored.join(", ")
            );
        }
...
fn run(mut cfg: RuntimeConfig, only: Selection, exec: Option<ExecPlan>) -> ExitCode {
    ...
    let result = runtime.block_on(async move {
        ...
        let storage_server = tokio::spawn(fireemu_adapter_http::storage_server::serve_storage(
            storage_listener,
            storage.clone(),
        ));
