"""Continuity approval is bound to one immutable receipt and 34 observations."""
...
def tool():
    path = Path(__file__).parents[1] / "auth-session-continuity-approval.py"
    assert path.exists(), "Subject-bound approval overlay required"
    spec = importlib.util.spec_from_file_location("continuity_approval", path)
...
def test_approval_page_is_current_and_bounded():
    ...
    assert "not a guarantee of permanent validity" in page
