...
| `FIREEMU_CONTROL_TOKEN`, `FIREEMU_CONTROL_URL` | always | token and URL, scoped to the direct `exec` child and Functions runner; the token is not printed |
...
The two background-trigger mutation routes and `POST /_admin/export` require the per-run control bearer. Browser preflights are admitted only from loopback origins for their exact method and `authorization` header, with Fetch Metadata checked when present; remote and opaque origins are never reflected. The private mode-`0600` Hub locator carries the capability for `fireemu emulators:export`, while public Hub discovery responses do not. The locator is written atomically without following symlinks and removed at shutdown.
...
- **`firestore`** in object or array (named-database) form: per-database `rules` and `indexes` (the legacy `index` spelling too);
...
Nothing is silently dropped. An `emulators.<name>` entry for a product fireemu does not serve is an **error** when `--only` named that product and a **notice** otherwise; anything fireemu applies only in part says so on stderr. Named Firestore databases load and enforce their own rules and index catalogs. A per-target Storage ruleset remains partial because one ruleset covers every bucket. `emulators.singleProjectMode` is recorded and published rather than enforced separately: fireemu already isolates every project into its own session, which is stricter.
...
The daemon prints the environment variables SDKs need (`FIRESTORE_EMULATOR_HOST`, `FIREBASE_AUTH_EMULATOR_HOST`, `FIREBASE_STORAGE_EMULATOR_HOST` / `STORAGE_EMULATOR_HOST`). Storage rules load from `storage.rules` in the config or `PUT /v1/storage/rules`. Firestore rules, Firestore indexes and Storage rules named by `firebase.json` are watched and replace the active generation only after the changed file parses completely; an invalid intermediate save retains the last-known-good generation. Security Rules also come from `rules.source` in the canonical config file or at runtime:
...
Snapshots copy what the session owns (its Firestore databases, Storage objects, Auth users, fault plan and text indexes) and, for the default session, the shared parts (the clock, both rulesets, the auto-ID generator) in one exclusive section (a restore of the default session is a new epoch: streams end, the functions runtime resets; outstanding functions work is not captured); they live in memory. A capture is all or nothing too: every part is copied before any of it is retained, so a store that cannot be read refuses the snapshot instead of retaining an empty part. A restore validates every part, takes a pre-image of every store and only then applies them in order; a store that refuses the apply is reported and the stores already written are put back, so the session is never half of one snapshot and half of another. Each session retains at most 16 named snapshots, and at most about 1 GiB of estimated retained bytes across them: a seventeenth name, or a capture whose parts would take the session past the byte budget, is `RESOURCE_EXHAUSTED` (429) and changes nothing, while capturing over a name the session already holds is always admitted (whatever its size) and releases the copy it held. What a snapshot retains is the visible state, not the running session: the Firestore part holds the newest version of every live document -- none of the MVCC history, open transactions or tombstones, so a `read_time` or resume token from before the restored state is refused exactly as after a compaction -- and the Storage part shares each object's bytes with the live store by reference, so capturing the same objects again, or restoring them, costs no copy of the data; only an overwrite allocates. The Auth part shares each unchanged user record with the live store by reference too: a capture copies only the users it must strip a TOTP secret from (default snapshots carry no shared secret), and a write after the capture clones just the one user it touches, so a retained snapshot is never changed by later activity. `GET /v1/sessions/{s}/snapshots` reports `retained`, `limit` and `remaining` names next to `retainedBytes`, `byteLimit` and `remainingBytes` (and per-snapshot `bytes`); the `POST` capture and `DELETE` responses report them too. Fault plans (spec 18) name an operation (`firestore.commit` / `read` / `beginTransaction`, `storage.upload` / `read` / `delete` / `list`, `functions.invoke` / `deliver`; a rule with only an `eventType` is a `functions.deliver` rule), optionally the nth occurrence (counted per function when one is named) and a function, and an action that applies to that operation (`returnError` with a gRPC name or HTTP code, `delay` seconds, `duplicate` count, `crashRunner`, `timeout`, `deadLetter`, `transactionConflict`, `dropConnection`); a combination the adapters would ignore is refused. `functions.invoke` rules also apply to HTTP invocations and to scheduled or manual runs; a `delay` holds an event until the virtual clock reaches the instant and then applies the other actions of the rule set. `dropConnection` closes the connection (or resets the gRPC stream) instead of answering on the Firestore, Storage and functions ports; over WebChannel it is reported as `UNAVAILABLE`, and for event invocations it is a failed attempt. `GET .../faultPlan` shows what fired.
...
The navigation separates the official Emulator products from the fireemu-only runtime controls: the virtual clock, snapshots, fault plans and sessions live under a labelled **fireemu runtime** group and the Runtime page says so, so the deterministic controls are never mistaken for official parity. The Overview carries a **Product scope** table that states every product's status explicitly rather than showing an empty tab: Auth, Firestore, Functions, Storage, Security Rules and App Check are supported, Realtime Database is **deferred**, Extensions is **not planned**, the official **Logging** emulator WebSocket (port 4500) is **served** by the daemon (the UI additionally streams the same logs over SSE), and the Firestore **Requests** trace, the **Rules coverage** report and the **Firebase alerts** workflow are official-parity views on the Rules and Alerts pages (UI-PARITY-03 / UI-PARITY-04). The Functions log stream is filterable in the browser by severity level and free text, and the invocation list by function.
...
Federated sign-in (`accounts:signInWithIdp`) is the fake identity provider the official emulator implements, not a real OAuth round trip: the credential is read from `requestUri` and/or `postBody` (as the SDKs send it, including the URI fragment), the `providerId` is lowercased, and the `id_token` — a fake JWT or a strict-JSON object with `sub` / `email` / `name` / `picture` — or a JSON `access_token` is trusted as the assertion. Each provider is shaped as the emulator shapes it: `google.com` reports the account URL as `federatedId` and a granted-scopes `rawUserInfo`; a `saml.<name>` provider reads the email from the `SAMLResponse` assertion subject `nameId` (a JSON blob, not real XML), forces `emailVerified`, and uses the assertion's `attributeStatements` as `rawUserInfo`; an `oidc.<name>` (or any other) provider keeps the claims as `rawUserInfo`. Create-or-link matches by `(providerId, rawId)`, then by a verified assertion email (linking, and recycling an unverified-email account by clearing its password, phone and other providers), else answers `needConfirmation` for an unverified assertion email an account already owns; an `access_token`-only credential is `NotImplemented` (501), as is `createAuthUri` with a `providerId`. The SDK-driven popup / redirect handoff pages are served at `/emulator/auth/handler` (the account picker for the provider) and `/emulator/auth/iframe` (the `gapi` relay), reproduced from the pinned `widget_ui.js`. Two behaviours here are deliberately stricter and recorded as `AUTH-OAUTH-0` divergences in the capability manifest: the widget HTML-escapes the account values it injects (the official page does not), and a `providerId` carrying a NUL, a control character or more than 256 bytes is refused before it reaches the store. The official emulator stores no SAML / OIDC provider configuration, so there is none to round-trip.

Identity Platform tenants have isolated users, tokens, refresh sessions and configuration behind the Admin v2 tenant CRUD routes, tenant-scoped Admin account routes and client `tenantId` routing. Tenant tokens carry `firebase.tenant` and cannot cross namespaces. Official Auth exports preserve each namespace as `auth_export/accounts-<tenant>.json`; default accounts remain in `accounts.json`. Tenant-specific UI and named snapshot management are not yet separate views.

Error shapes follow the official emulator, including its documented differences from production: by default a password sign-in distinguishes `EMAIL_NOT_FOUND` from `INVALID_PASSWORD` and a password reset for an unknown address is `EMAIL_NOT_FOUND`, while Admin v2 `emailPrivacyConfig.enableImprovedEmailPrivacy` collapses both into `INVALID_LOGIN_CREDENTIALS`, answers unknown reset and verification requests silently, hides `createAuthUri`'s sign-in methods, and requires a verified OOB flow instead of direct self-service email changes. Linking an email/password credential under that policy uses `accounts:signUp` with the current `idToken`, as Identity Platform specifies. `signIn.allowDuplicateEmails` applies to IdP accounts while password accounts remain unique. Password reset, email verification, email-link sign-in and verify-before-change actions issue inspectable OOB links under `/emulator/v1/projects/{p}/oobCodes`. Phone second-factor enrollment refuses an anonymous, phone, custom-token or Game Center first factor (`UNSUPPORTED_FIRST_FACTOR`), an unverified email (`UNVERIFIED_EMAIL`) and a number already enrolled (`SECOND_FACTOR_EXISTS`) without creating or consuming a verification code; the pending-credential response obfuscates the number to its last four digits. TOTP enrollment issues a Base32 shared secret and session, and TOTP sign-in verifies RFC 6238 codes. TOTP factors (`AUTH-MFA-TOTP-1`) are a fireemu addition the official emulator has no route for, so the verified-email requirement is not inferred for them.
...
| `auth_export/` | `accounts.json`, `accounts-<tenant>.json`, `config.json` | every default and tenant account with its providers, custom claims, phone and TOTP second factors, and the project's Auth configuration |
...
**Import and export resource limits.** The root manifest is limited to 4 MiB, other metadata files to 64 MiB, and Firestore output to 1 GiB across all databases in one artifact. Auth permits at most 64 MiB per file, 256 MiB across the section and 2,048 section entries. Storage permits at most 1 GiB across the section, 10,000 objects, 25,000 directory entries and four directory levels. Import files are opened relative to the artifact directory without following any path component and their opened descriptors are size-checked before allocation. These limits are checked during preparation before any product state is installed; diagnostics name the stable limit and path without echoing artifact contents. Replacing an export preserves unmanaged regular entries only up to 1 GiB, 25,000 entries and 64 directory levels; exceeding a preservation limit leaves the existing export unchanged.
...
`--functions <dir>` (or `functions.source` in the config) starts `tools/runner-node/index.mjs` (Node, needs the codebase's own `node_modules` with `firebase-functions` and `firebase-admin`; `express` comes with `firebase-functions`). The runner discovers the exported v2 functions and reports them; the daemon then delivers Firestore document events (`onDocumentCreated` / `Updated` / `Deleted` / `Written`, including auth-context variants and named databases), Storage object events (`onObjectFinalized` / `Deleted` / `MetadataUpdated` / `Archived`), Auth lifecycle events, synchronous Identity Platform `beforeUserCreated` / `beforeUserSignedIn`, Pub/Sub, Cloud Tasks, Eventarc custom events, Firebase alerts and `onSchedule` runs, and serves `onRequest` / `onCall` at `http://127.0.0.1:5001/{project}/{region}/{function}`. Blocking identity responses apply `displayName`, `photoURL`, `disabled`, `emailVerified`, `customClaims` and before-sign-in `sessionClaims` before the final token is issued; the Auth store lock is not held while user code runs, and a rejection rolls the candidate state back. Functions declared with `retry: true` are retried with exponential backoff in virtual time; other failures are dead-lettered. Realtime Database and Remote Config triggers remain deferred because those product emulators are not served.
...
**Environment, parameters and reload.** A codebase's `.env`, `.env.<projectId>`, `.env.<alias>` and `.env.local` are read in that order and in the dialect `firebase-tools`' own `lib/functions/env.js` defines, refusals included: a reserved key, a key that is not `SCREAMING_SNAKE`, a key under `X_GOOGLE_` / `FIREBASE_` / `EXT_` / `KIT_`, a line that is not an assignment, or both a project-ID and an alias file present, each stop the run with the sentence the Firebase CLI prints. `.secret.local` supplies `defineSecret` values, but a value is inserted into `process.env` only while a function that declared that secret is running; discovery and functions without the binding cannot read it. Because one fireemu runner serves a whole codebase and Node's environment is process-global, a codebase with local secrets serializes handler entry while setting and restoring those values. `.runtimeconfig.json` becomes `CLOUD_RUNTIME_CONFIG` (the pinned `firebase-functions@7` has removed `functions.config()`, so nothing in the supported SDK reads it). `defineString` / `defineInt` / `defineBoolean` / `defineList` resolve from the non-secret environment exactly as they do under the official emulator, which resolves no parameters and prompts for nothing: a value nothing defines is `""`, `0`, or a `defineList` that throws, and a declared `default` is a deploy-time value the runtime never sees. Source files, compiled output and those environment/configuration files are watched; after a debounced change, a fresh runner must complete discovery before it atomically replaces the old generation. A syntax/load failure or a changed trigger manifest retains the last-known-good generation; adding or removing an exported trigger still requires a daemon restart. `functions.ignore`, `node_modules` and `.git` are excluded from the signature. `node_modules` remains linked into immutable source snapshots, so installing or changing dependencies also requires a daemon restart. Other source symlinks are rejected instead of being silently omitted from a generation. The runtime is started with `FIREBASE_CONFIG` (`storageBucket`, `databaseURL`, `projectId`), `GCLOUD_PROJECT`, `GOOGLE_CLOUD_QUOTA_PROJECT`, `FUNCTIONS_EMULATOR`, `K_REVISION`, `PORT`, `TZ=UTC` and `METADATA_SERVER_DETECTION`, plus `FUNCTION_TARGET`, `FUNCTION_SIGNATURE_TYPE` and `K_SERVICE` set per invocation — the official emulator runs one process per trigger and sets those three once, and one fireemu runner serves a whole codebase, so they name the most recently started invocation.

**2nd-gen options.** Runtime-relevant `GlobalOptions` are enforced locally: `region` routes the endpoint, `timeoutSeconds` sets the deadline, `concurrency` limits parallel invocations, `enforceAppCheck` controls callable admission, `secrets` are sourced from `.secret.local`, and `omit` removes the export. Managed-deployment options have no honest local analogue, so `memory`, `cpu`, `minInstances`, `maxInstances`, `ingressSettings`, `invoker`, `serviceAccount`, VPC connector/egress or direct network interface, `labels` and `preserveExternalChanges` are retained under `platformOptions` in status output but do not emulate Cloud Run resources, IAM, networking or deployment merge behavior. Every `ScheduleOptions` retry field (`retryCount`, `maxRetrySeconds`, `minBackoffSeconds`, `maxBackoffSeconds`, `maxDoublings`) drives virtual-time retries. `timeZone` accepts IANA zones and is tested across daylight-saving gaps and folds; an invalid zone fails discovery.
...
Browser pages on a loopback origin must send `Authorization: Bearer <control token>` to privileged control routes (reset, clock, rules, functions, `awaitIdle`). The served Emulator UI receives the per-run token in its same-origin bootstrap document, and a direct `exec` child receives it as `FIREEMU_CONTROL_TOKEN`; normal logs do not expose it. Command-line clients need no token.

Pub/Sub topic triggers (`onMessagePublished`, v1 `topic().onPublish`) receive messages published through the control API (`POST /v1/sessions/default/pubsub/topics/{topic}:publish` with `{"messages": [{"json": {...}, "attributes": {...}}]}`, or the Pub/Sub REST shape `/v1/projects/{project}/topics/{topic}:publish` with base64 `data`). Auth user created / deleted events reach v1 `auth.user().onCreate` / `onDelete` handlers. `*WithAuthContext` Firestore triggers carry `authtype` / `authid` of the principal that committed. `scheduler.catchUp` chooses what happens to schedule runs that became due while the clock moved: `all` (default), `latest` (one run per job), `none`. `latest` and `none` compute the run they keep directly instead of walking the missed ones, so a jump of years costs the same as a jump of minutes; what they drop appears as one `skipped: catch-up ...` record per job and clock change, carrying a count that is exact up to `scheduler.maxCatchUpRuns` (1000) and reported as `at least N runs` beyond it (an `every N minutes` schedule is always counted exactly). `tools/sdk-smoke/functions.mjs` with `tools/sdk-smoke/functions-project/` exercises the supported triggers and options (pin the clock with `--config tools/sdk-smoke/fireemu.smoke.json`: schedule counts depend on it).
...
(`conformance/src/firestore-probe/divergences.mjs`, 15 rows) and published in
`spec/compatibility/contract.json`. Transactions use the same pessimistic concurrency as the
official emulator: a read-write transaction locks the documents it reads and the collections
its queries scan, and a write against a locked document waits and is then refused with
`ABORTED` "Transaction lock timeout." while the transaction commits (the wait runs on the
virtual clock, not wall time, so it stays deterministic). The remaining pinned rows are
official REST-adapter defects fireemu does not reproduce, official leniencies it refuses (a
selector-less query, a write addressed to another database than the route, an uppercase
database id), one official aggregation-counting defect, and two capabilities fireemu serves
that the official emulator refuses (`orderBy(__name__, desc)` and PartitionQuery).
