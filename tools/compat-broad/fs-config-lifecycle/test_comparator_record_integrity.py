...
OTHER_NONCE = "b" * 32
ORDER = (
    "OC-01", "OC-02", "OC-13", "OC-14", "OC-22", "OC-15", "OC-16",
    "OC-17", "OC-18", "OC-19", "OC-20", "OC-21",
)
...
def compiler(monkeypatch):
    monkeypatch.setattr(cmp, "compile_cases", lambda nonce: [
        {"id": name, "expectedLocal": {
...
def collection():
    rows = [{
        "index": i, "phase": "observation", "role": "case", "case": name,
        "complete": True, "status": 200, "typedError": None,
        "shape": {"state": {"enum": "READY"}}, "failure": None,
    } for i, name in enumerate(ORDER)]
    return {
        "campaignId": cmp.CASE_ID, "nonceDigest": digest(NONCE),
        "completed": True, "cleanupComplete": True,
        "stopPoint": None, "failure": None,
...
def comparison(monkeypatch, local=None, production=None, *, nonce=NONCE, local_nonce=None):
    ...
    acquisition = cmp.VerifiedAcquisition(
        campaign_id=cmp.CASE_ID, execution_kind=cmp.PRODUCTION_KIND,
        endpoint=cmp.PRODUCTION_ORIGIN, reservation="a" * 64,
        ledger_identity="unit-test-only", receipt_digest="b" * 64,
        gate_digest="c" * 64, artifact_sha256="d" * 64,
        worker_sha256="e" * 64, collection_digest=digest(production),
        synthetic=True,
    )
    monkeypatch.setattr(cmp, "_registered", lambda value: value is acquisition)
    return cmp.compare(
        _manifest(nonce), {"executionKind": cmp.LOCAL_KIND, "collection": local},
        {"executionKind": cmp.PRODUCTION_KIND, "collection": production}, nonce,
        acquisition=acquisition, local_nonce=local_nonce,
    )
...
def test_both_saved_runs_cannot_be_joined_to_a_different_manifest(monkeypatch):
    ...
    assert_unknown(result)
    assert set(result["errors"]) == {"local-collection-nonce", "production-collection-nonce"}


@pytest.mark.parametrize("at_end", [False, True])
@pytest.mark.parametrize("changed", [False, True])
def test_duplicate_observation_is_never_resolved_by_selecting_first(monkeypatch, at_end, changed):
    col = collection()
...
@pytest.mark.parametrize("key,value", [
    ...
    ("failure", "TimeoutError"), ("stopPoint", "cancelled"),
    ("cleanupComplete", False), ("cleanupComplete", 1),
])
...
@pytest.mark.parametrize("key", ["completed", "failure", "stopPoint", "cleanupComplete"])
...
@pytest.mark.parametrize("key", ["failure", "shape", "typedError", "complete", "status"])
...
@pytest.mark.parametrize("typed_error", [
    [], 1, "error", {"code": True, "status": "UNIMPLEMENTED"},
    {"code": 501}, {"code": 501, "status": []},
])
...
def test_ordinary_typed_refusal_can_still_match(monkeypatch):
    ...
    for col in (left, right):
        col["rows"][0].update(status=404, typedError={"code":404,"status":"NOT_FOUND"})
...
def test_expected_local_deviation_is_not_whole_run_match(monkeypatch):
    col = collection()
    col["rows"][8].update(status=501, typedError={"code":501,"status":"UNIMPLEMENTED"})
    result = comparison(monkeypatch, col)
...
def test_deviation_needs_the_exact_declared_refusal(monkeypatch, status, code):
    col = collection()
    col["rows"][8].update(status=status, typedError={"code":code,"status":"UNIMPLEMENTED"})
    result = comparison(monkeypatch, col)
    assert result["classification"] == cmp.MISMATCH
...
@pytest.mark.parametrize("key,value", [
    ("index", True), ("index", 99), ("phase", "missing"),
    ("role", []), ("role", "unknown"), ("case", []),
])
...
def test_missing_acquisition_is_still_refused():
    col = collection()
    result = cmp.compare(_manifest(NONCE),
        {"executionKind":cmp.LOCAL_KIND,"collection":col},
        {"executionKind":cmp.PRODUCTION_KIND,"collection":copy.deepcopy(col)},NONCE)
...
def test_manifest_boolean_does_not_equal_numeric_version(monkeypatch):
    col=collection()
    bad=_manifest(NONCE);bad["version"]=True
    result=cmp.compare(bad,{"executionKind":cmp.LOCAL_KIND,"collection":col},
                       {"executionKind":cmp.PRODUCTION_KIND,"collection":col},NONCE)
    assert result["errors"] == ["manifest-drift"]


@pytest.mark.parametrize("value", [float('nan'), float('inf'), {1:'non-string-key'}, ()])
def test_non_json_input_is_refused_before_digest(value):
    col=collection();col['rows'][0]['shape']=value
    rows=cmp.compare_rows(col,collection(),NONCE)
    assert {row['classification'] for row in rows} == {cmp.INDETERMINATE}


def test_cyclic_input_is_bounded():
    col=collection();col['cycle']=col
    rows=cmp.compare_rows(col,collection(),NONCE)
    assert all(r['classification'] == cmp.INDETERMINATE for r in rows)


def test_empty_run_is_not_a_vacuous_match(monkeypatch):
    col=collection();col['rows']=[];col['rowCount']=0
    assert_unknown(comparison(monkeypatch,col,col))


def test_more_rows_than_gate_ceiling_are_unknown(monkeypatch):
    col=collection();col['rows']=[{**col['rows'][0],'role':'poll'} for _ in range(cmp.MAX_REQUESTS+1)]
