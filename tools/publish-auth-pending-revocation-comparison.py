"""Publish the local-versus-production comparison of the pending-revocation corpus.
...
RECEIPT = ROOT / "spec/compatibility/evidence/auth-pending-revocation/receipt.json"
BUNDLE = (
    ROOT / "spec/compatibility/evidence/auth-pending-revocation/local-comparison.json"
)
PAGE = ROOT / "docs/compatibility/auth-pending-revocation-comparison.md"
...
def project_local(report, recorder_commit, runtime_commit):
    require(
        ...
        == digest(
            {"slice": "auth-pending-revocation", "revision": 1, "cases": list(CASES)}
        )
        and complete(report)
    )
...
def validate_local(local):
    ...
    times = local["heldFinalizeTimes"]
    if local["cases"][3]["outcome"] == "accepted":
