...
@pytest.mark.skipif(
    os.getenv("AUTH_PASSWORD_LIVE_LOCAL") != "1", reason="owned process opt-in"
)
def test_password_owned_lifecycle(tmp_path):
    path = Path(__file__).with_name("password_owned.py")
    ...
    from password_contract import CASES, complete, validate_case
    from password_recorder import recovery_identity
