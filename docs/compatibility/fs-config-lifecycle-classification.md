...
| `databases.get` | Client tooling and the emulator suite read database settings before choosing a data-plane code path, and several returned fields change what the data plane accepts, so the projection is part of the contract rather than provisioning. | A wrong concurrencyMode, databaseEdition or versionRetentionPeriod makes a caller choose transaction, edition and read-time behavior the local runtime will then refuse or accept differently from production. | `crates/fireemu-adapter-grpc/src/rest/mod.rs:692` `crates/fireemu-adapter-grpc/src/rest/mod.rs:767` `crates/fireemu-adapter-grpc/src/rest/mod.rs:278` |
| `databases.list` | Enumeration decides whether a named database exists at all, which is the precondition every data-plane request on that database depends on. | A database absent in production but present locally lets a caller address a database that would return NOT_FOUND against the real service. | `crates/fireemu-adapter-grpc/src/rest/mod.rs:692` `crates/fireemu-adapter-grpc/src/rest/mod.rs:756` `crates/fireemu-adapter-grpc/src/rest/mod.rs:735` |
