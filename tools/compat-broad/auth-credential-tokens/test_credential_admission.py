...
class Admission:
    ...
    def __init__(self, tmp_path: Path, *, signing: bool = True):
        ...
        self.handoff = {
            ...
            "signing": {"serviceAccount": campaign.SERVICE_ACCOUNT} if signing else None,
        }
...
def test_the_budget_is_the_lane_budget_with_management_slots_inside_it() -> None:
    ...
    for signing, management in ((True, 7), (False, 4)):
        ...
        assert reference["bounds"]["observationRequests"] + reference["bounds"]["cleanupRequests"] + management <= 60
...
def test_the_plan_reference_names_exactly_one_gate_plan() -> None:
    ...
    for drift in ({"signing": False}, {"planDigest": "0" * 64}, {"nonce": "f" * 32}, {"runnableCases": []}):
...
def test_lock_scopes_name_the_accounts_the_config_reads_and_the_signer() -> None:
    ...
    assert keys[f"project/fireemu-35fe6/auth/accounts/fireemu-cred-{NONCE[:8]}-0"] == "WRITE"
    ...
    assert keys[f"project/fireemu-35fe6/iam/serviceAccounts/{campaign.SERVICE_ACCOUNT}/signBlob"] == "READ"
    for lock in locks:
    ...
    assert not any("signBlob" in lock["key"] or "custom-" in lock["key"] for lock in without)
...
def test_frozen_inputs_pass_the_shared_admission_check_set(built) -> None:
    ...
    cap = admission.issue_production_capability(
        **built.bindings(), binding=campaign.remote.worker_binding()[0], binding_digest=campaign.remote.worker_binding()[1]
    )
...
def test_an_approval_for_another_campaign_is_refused(built) -> None:
    with pytest.raises(ValueError, match="another campaign"):
        admission.validate_o7_admission(**built.bindings(approval={**built.approval, "campaignId": "FS-LIMIT-API-REQUEST-BYTES"}))
...
def test_binding_drift_in_the_approval_is_refused(built, field, value, match) -> None:
    with pytest.raises(ValueError, match=match):
        admission.validate_o7_admission(**built.bindings(approval={**built.approval, field: value}))


def test_a_permission_missing_the_frozen_auth_config_digest_is_refused(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="Auth config digest"):
        admission.freeze_inputs(built.permission_path, built.plan, source_root=built.source, artifact_path=built.artifact_path)
...
def test_a_modified_frozen_source_is_refused_by_provenance(built) -> None:
    ...
    with pytest.raises(ValueError, match="clean frozen source"):
        admission._provenance(built.source, built.inputs["sourceCommit"], built.inputs["sourceInputs"])


def test_the_frozen_inputs_name_the_collector_comparator_worker_and_transport(built) -> None:
    sources = built.inputs["sourceInputs"]
    for entry in (campaign.COLLECTOR_ENTRY, campaign.COMPARATOR_ENTRY, campaign.WORKER_ENTRY, campaign.TRANSPORT_ENTRY, campaign.GATE_ENTRY):
    ...
    assert set(generation["sourceDigests"]) == {"shared_gate.py", "reservations.py", "o8_admission.py", "credential_admission.py", "credential_descriptor.py"}
...
@pytest.mark.parametrize(
    "mutate,match",
    [
        (lambda h: h.update(kind="request-bytes-bearer-token-v1"), "bound credential handoff"),
        ...
        (lambda h: h.update(signing={"serviceAccount": "other@example.invalid"}), "campaign service account"),
        (lambda h: h.update(signing={"serviceAccount": campaign.SERVICE_ACCOUNT, "key": "x"}), "campaign service account"),
...
def test_a_signing_declaration_is_refused_against_a_plan_frozen_without_it(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="differs from the frozen plan"):
        admission.validate_handoff({**built.handoff, "signing": {"serviceAccount": campaign.SERVICE_ACCOUNT}}, built.permission, built.plan)
    accepted = admission.validate_handoff(built.handoff, built.permission, built.plan)
...
def test_the_reservation_claim_binds_the_gate_plan_and_the_lane_budget(built, tmp_path) -> None:
