...
def _valid(record: Any, side: str) -> bool:
    ...
    if not isinstance(binding, dict) or not isinstance(binding.get("commit"), str) or not isinstance(binding.get("artifactSha256"), str):
        return False
    if not re.fullmatch(r"[0-9a-f]{40}", binding["commit"]) or not re.fullmatch(r"[0-9a-f]{64}", binding["artifactSha256"]):
        return False
    ...
    if not isinstance(stages, list) or [item.get("id") for item in stages if isinstance(item, dict)] != list(STAGE_IDS) or len(stages) != len(STAGE_IDS):
        return False
    if not all(isinstance(item.get("response"), dict) and type(item["response"].get("status")) is int for item in stages):
        return False
    ...
    if not isinstance(state, dict) or not all(isinstance(state.get(key), dict) for key in ("afterWrong", "afterSuccess")) or not isinstance(recovery, dict):
        return False
    return recovery.get("ownerVerified") is True and recovery.get("cleanupVerified") is True and type(recovery.get("remainingAccounts")) is int and recovery["remainingAccounts"] == 0
...
def compare(left: dict, right: dict) -> dict:
    ...
    return {"classification": classification, "receiptStatus": "STRUCTURALLY_COMPLETE_UNBOUND" if structurally_complete else "INCOMPLETE", "productionExecuted": False, "normalizedDigest": digest}
