...
use ftd_adapter_http::identity_toolkit::{handle_with, AuthState, JsonResponse, RequestHeaders};
...
fn an_enforced_mfa_finalize_without_app_check_does_not_consume_the_assertion() {
    ...
    let key = ftd_core_auth::base32::decode(&secret).expect("the shared secret is base32");
...
async fn duplicate_app_check_fields_over_a_real_socket_are_never_admitted() {
    ...
    let server = tokio::spawn(ftd_adapter_http::server::serve(listener, h.auth.clone()));
