...
- Changed, strict profile: a write whose precondition fails is refused with that failure (`ALREADY_EXISTS`, `NOT_FOUND`, `FAILED_PRECONDITION`) before Security Rules are read, as production answers; the emulator profile keeps the official emulator's order, which answers the rule's denial first.
