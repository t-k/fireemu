...
use fireemu_core_app_check::observe::CredentialCategory;
...
fn enforced_denies_a_missing_token_with_the_required_code() {
    ...
    assert_eq!(
        decision.reason,
        Some(fireemu_core_app_check::verify::PUBLIC_REQUIRED_REASON)
    );
}
...
fn enforced_denies_every_invalid_credential_with_one_public_code() {
    ...
    for header in &cases {
        ...
        assert_eq!(
            decision.reason,
            Some(fireemu_core_app_check::verify::PUBLIC_DENIAL_REASON),
