...
import { requireThat, equal, sha256, digestJson, blobSha, safeCode, validateProgram } from './core.mjs';
...
const program = validateProgram(JSON.parse(await fs.readFile(join(directory, 'program.json'))), entry);
...
for (const [name, hash] of [['session.mjs', entry.sessionBlob], ['credentials.mjs', entry.credentialsBlob]])
...
for (const key of Object.keys(process.env)) if (key.startsWith('FIRESTORE_PROBE_')) delete process.env[key];
Object.assign(process.env, {
  FIRESTORE_PROBE_HOST: new URL(origin).host, FIRESTORE_PROBE_SCHEME: 'http',
  FIRESTORE_PROBE_TARGET: 'local', FIRESTORE_PROBE_TOKEN: 'owner',
  FIRESTORE_PROBE_PROJECT: entry.project, FIRESTORE_PROBE_RECORD_PROJECT: entry.project,
  FIRESTORE_PROBE_IN: join(directory, 'programs.json'),
  FIRESTORE_PROBE_OUT: join(directory, 'local.json'), FIRESTORE_PROBE_TIMEOUT_MS: '10000',
});
...
const expected = [
  ...
  ...program.seed.map(seed => ({ phase: 'seed', method: 'PATCH',
    path: seed.path.replaceAll('PROJECT', entry.project), body: { fields: replace(seed.fields) } })),
  ...program.steps.map(step => ({ phase: step.id, method: step.method,
    path: step.path.replaceAll('PROJECT', entry.project),
    ...(step.body === undefined ? {} : { body: replace(step.body) }) })),
];
const requests = []; let index = 0, totalBytes = 0;
globalThis.fetch = async (url, init = {}) => {
  const parsed = assertUrl(url, origin);
  const op = expected[index];
  requireThat(op && op.method === (init.method ?? 'GET') && op.path === parsed.pathname + parsed.search,
    'unexpected-recorder-operation');
  const body = init.body === undefined ? undefined : JSON.parse(init.body);
  requireThat((op.body === undefined && body === undefined) ||
    (op.body !== undefined && body !== undefined && equal(op.body, body)), 'recorder-input-drift');
  const authorization = new Headers(init.headers).get('authorization');
  requireThat(authorization === (op.phase === 'reset' ? null : 'Bearer owner'), 'recorder-principal-drift');
  index++;
  const row = { phase: op.phase, method: op.method, path: op.path,
    bodySha256: init.body === undefined ? null : sha256(init.body), status: null };
  requests.push(row);
  const response = await guardedFetch(url, { ...init, redirect: 'error' });
  const text = await boundedText(response);
  totalBytes += Buffer.byteLength(text);
  requireThat(totalBytes <= 8 * 1024 * 1024, 'response-budget');
  row.status = response.status; row.responseSha256 = sha256(text);
...
try {
  ...
  try {
    ...
    cleanup.requests++;
    const response = await guardedFetch(origin + clearPath, { method: 'DELETE', signal: AbortSignal.timeout(10000) });
    await boundedText(response); requireThat(response.ok, 'cleanup-reset-failed');
    for (const path of entry.ownedDocuments) {
      cleanup.requests++;
      const r = await guardedFetch(`${origin}/v1/projects/${entry.project}/databases/(default)/documents/${path}`,
        { headers: { authorization: 'Bearer owner' }, signal: AbortSignal.timeout(10000) });
      const body = JSON.parse(await boundedText(r));
      requireThat(r.status === 404 && body?.error?.status === 'NOT_FOUND', 'cleanup-absence-unconfirmed');
      cleanup.absent.push(path);
    }
    cleanup.state = 'confirmed';
  } catch (error) { cleanup.failure = safeCode(error); }
  ...
  await publishJson(join(directory, 'session-result.json'), {
    ...
    programDigest: digestJson(program), localSha256: local ? sha256(local) : null,
    completed: !failure && index === expected.length, failure, cleanup,
    requests, requestCount: index, totalResponseBytes: totalBytes,
    endpoint: origin, productionRequests: 0,
  });
}
