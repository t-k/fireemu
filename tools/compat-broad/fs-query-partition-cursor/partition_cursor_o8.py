"""Bounded O8 entrypoint for one already approved partition/cursor campaign.
...
before it reads the private handoff. Exit 0 requires verified cleanup and
Ledger release; exit 1 means the run did not complete and the reservation is
still held with a receipt that names its disposition; exit 2 refuses admission
before anything was created.

The only production origin this launcher can reach is the fixed one the lane's
wire module names; a loopback origin is refused there by construction.
"""
...
HANDOFF_KIND = "partition-cursor-bearer-token-v1"


class CredentialHandoffRefused(ValueError):
    """The private handoff did not carry a token bound to this permission.

    Named so a receipt written after the reservation records the refusal
    class rather than a generic error; nothing has been sent at that point.
    """


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Execute one frozen, O7-approved partition/cursor campaign."
    )
...
def validate_handoff(handoff: dict, permission: dict) -> str:
    ...
    if (
        ...
        raise CredentialHandoffRefused(
            "bound partition-cursor credential handoff required"
        )
    token = handoff["token"]
    if (
...
def execute(args: argparse.Namespace) -> dict:
    ...
    try:
        ...
        from partition_cursor_production import execute as execute_production
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Partition/cursor O8 {state} ({type(error).__name__}).", file=sys.stderr)
        return 1 if uncertain else 2
    if result.get("reservationReleased") and result.get("failure") is None:
        return 0
    # The reservation is still held either way; the receipt names whether it
    # can be retired as no data, closed after an abandoned cleanup, or needs
    # the owner. Nothing is retired here.
    return 1
