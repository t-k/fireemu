...
// documented cases are asserted; the primary-key cases are recorded as evidence.
//
...
async function probe(name, expected, fn) {
  ...
  results.push({ name, expected, outcome, detail, matches: expected === null ? null : expected === outcome });
}
...
try {
  ...
  await probe("mrg: category == AND city == orderBy star_rating (merge two composites)", "ok", async () =>
    ids(await mrg.where("category", "==", run).where("city", "==", "SF").orderBy("star_rating").get()));
  await probe("mrg: category == AND city == (merge automatic indexes)", "ok", async () =>
    ids(await mrg.where("category", "==", run).where("city", "==", "SF").get()));
  await probe("mrg: + editors_pick == orderBy star_rating (no editors_pick composite)", "9", async () =>
    ids(await mrg.where("category", "==", run).where("city", "==", "SF").where("editors_pick", "==", true).orderBy("star_rating").get()));
  await probe("mrg: category == AND city == orderBy star_rating DESC (direction mismatch)", "9", async () =>
    ids(await mrg.where("category", "==", run).where("city", "==", "SF").orderBy("star_rating", "desc").get()));
  await probe("mrg: category == orderBy star_rating (single composite)", "ok", async () =>
    ids(await mrg.where("category", "==", run).orderBy("star_rating").get()));

  await probe("pk: n == 1 (exempt field, control)", "9", async () =>
    ids(await pk.where("n", "==", 1).get()));
  await probe("pk: __name__ == ref", null, async () =>
    ids(await pk.where(FieldPath.documentId(), "==", pk.doc(`${run}-a`)).get()));
  await probe("pk: __name__ in [refs]", null, async () =>
