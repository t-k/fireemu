...
impl AppCheckRsaSigner {
    ...
    pub fn generate(source: AppCheckKeySource) -> Result<Arc<Self>, String> {
        ...
        let kid = digest.iter().take(8).fold(
            String::from(ftd_core_app_check::crypto::KEY_ID_PREFIX),
            |mut acc, b| {
                use std::fmt::Write as _;
                let _ = write!(acc, "{b:02x}");
                acc
    ...
    #[must_use]
    pub fn jwks(&self) -> serde_json::Value {
        use ftd_core_app_check::crypto::AppCheckSigner as _;
        let key: serde_json::Value =
            serde_json::from_str(&self.public_jwk_json()).unwrap_or_default();
        serde_json::json!({"keys": [key]})
    }
...
impl ftd_core_app_check::crypto::AppCheckSigner for AppCheckRsaSigner {
    fn alg(&self) -> &'static str {
        "RS256"
    }
...
fn derived_seed(seed: u64) -> [u8; 32] {
    use ftd_core_types::determinism::DeterministicRng as _;
    let mut rng = ftd_core_types::determinism::SplitMix64::new(seed ^ APP_CHECK_DOMAIN);
    let mut bytes = [0u8; 32];
...
impl ftd_core_app_check::crypto::DebugTokenHasher for Sha256DebugTokenHasher {
...
impl ftd_core_app_check::crypto::ConstantTimeEq for SubtleConstantTimeEq {
