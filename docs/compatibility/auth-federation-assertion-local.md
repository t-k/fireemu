...
| Trust scope and fixture downgrade | `signed_oidc_rejects_fixture_downgrades_and_wrong_trust_scope` | Project/tenant/provider/issuer/client/kid mismatch, disabled provider and fake JSON/unsigned tokens refused. Tenant routing success remains debt. |
...
Should Fix: Before activating this in the daemon, define secure trust provisioning/rotation and exercise the real SDK and routed tenant paths. Keep advanced credential flows outside any claimed parity until separately tested.

Notes: The requested `~/.agents/agents/security_specialist.md` was absent; the installed `~/.claude/agents/security-reviewer.md` checklist was used for the implementation review. Trust contains public key material only. The verifier never follows JWT header URLs. Default fixture mode is explicit and remains separate. An independent parent review is required before integration.
