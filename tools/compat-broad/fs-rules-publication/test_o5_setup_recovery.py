...
def fixture(monkeypatch, fault=None):
    shadow = module.LocalShadow("http://127.0.0.1:18081", "http://127.0.0.1:18082", "demo-local", NONCE)
    ...
    def request(method, url, body=None, credential=None):
        ...
        if ":signUp" in path:
            ...
            if body.get("email"): users[uid]["email"] = body["email"]
            if fault == "lost-signup" and counts["signup"] == 2: raise module.Refused("lost")
            return 200, {"localId": uid, "idToken": "local-opaque"}
        if ":update" in path:
            if fault == "claims": raise module.Refused("claims-failure")
            return 200, {}
        ...
        if method == "PATCH":
            assert urllib.parse.parse_qs(urllib.parse.urlsplit(url).query) == {"currentDocument.exists": ["false"]}
            counts["patch"] += 1
            if fault == "conflict" and counts["patch"] == 3:
                documents[resource] = {"name": resource, "fields": {"foreign": {}}, "updateTime": VERSION}
                return 409, {"error": {"code": 409, "status": "ALREADY_EXISTS"}}
            documents[resource] = {"name": resource, "fields": body["fields"], "updateTime": VERSION}
            if fault == "lost-patch" and counts["patch"] == 3: raise module.Refused("lost")
            if fault == "bad-ack" and counts["patch"] == 3: return 200, {"name": resource}
            return 200, documents[resource].copy()
        if method == "GET":
            if resource in documents: return 200, documents[resource].copy()
            return 404, {"error": {"code": 404, "status": "NOT_FOUND"}}
        if method == "DELETE":
            assert urllib.parse.parse_qs(urllib.parse.urlsplit(url).query)["currentDocument.updateTime"] == [documents[resource]["updateTime"]]
...
@pytest.mark.parametrize("fault", ["claims", "lost-signup", "lost-patch", "bad-ack", "conflict"])
def test_partial_setup_recovers_only_acknowledged_owned_resources(monkeypatch, fault):
    ...
    with pytest.raises(module.Refused): shadow.setup()
    before = len(calls)
...
def test_complete_setup_can_be_rolled_back_with_versions_and_typed_final_reads(monkeypatch, tmp_path):
    ...
    journal = [json.loads(line) for line in shadow.setup_journal.read_text().splitlines()]
...
def test_pending_create_stays_pending_even_if_subsequent_read_would_be_absent(monkeypatch):
    shadow, users, documents, calls = fixture(monkeypatch, "lost-patch")
    with pytest.raises(module.Refused): shadow.setup()
    lost = [resource for resource, proof in shadow.setup_documents.items() if proof["outcome"] == "unknown"][0]
    documents.pop(lost)
    result = shadow.recover_setup()
    assert result["complete"] is False
...
def test_second_rollback_cannot_refill_request_or_time_reserves(monkeypatch):
    ...
    with pytest.raises(module.Refused, match="already-started"): shadow.recover_setup()
    assert calls == []
...
def test_setup_recovery_refuses_unbounded_or_malformed_deadline(monkeypatch, deadline):
    shadow, _, _, calls = fixture(monkeypatch)
    with pytest.raises(module.Refused): shadow.recover_setup(deadline_seconds=deadline)
    assert calls == []


def test_insufficient_remaining_time_does_not_start_a_recovery_request(monkeypatch):
    shadow, users, documents, calls = fixture(monkeypatch)
    shadow.setup(); before = len(calls)
...
def test_lost_or_malformed_tenant_creation_never_means_no_tenant_effect(monkeypatch, shape):
    shadow, _, _, calls = fixture(monkeypatch)
    def create(*_a, **_k):
        if shape == "lost": raise module.Refused("lost")
        name = {"foreign": "projects/other/tenants/t", "empty": "", "segment": "projects/demo-local/tenants/a/b"}[shape]
        return 200, {"name": name}
    monkeypatch.setattr(module, "_request", create)
    with pytest.raises(module.Refused): shadow.create_tenant()
    assert shadow.delete_tenant() is False


@pytest.mark.parametrize("fault", [None, "present", "forbidden", "wrong-message", "bad-code", "delete-error", "status-conflict"])
def test_tenant_deleted_requires_typed_final_absence(monkeypatch, fault):
    shadow, _, _, _ = fixture(monkeypatch)
    shadow.tenant = "owned-tenant"; shadow.tenant_attempted = True
    calls = []
    def request(method, url, *_a):
        calls.append(method)
        if method == "DELETE": return (200, {"error": {}}) if fault == "delete-error" else (200, {})
        body = {"error": {"code": 404, "message": "TENANT_NOT_FOUND"}}
        if fault == "present": return 200, {"name": "projects/demo-local/tenants/owned-tenant"}
        if fault == "forbidden": return 403, body
        if fault == "wrong-message": body["error"]["message"] = "Not Found"
        if fault == "bad-code": body["error"]["code"] = "404"
        if fault == "status-conflict": body["error"]["status"] = "PERMISSION_DENIED"
        return 404, body
    monkeypatch.setattr(module, "_request", request)
...
def test_child_executes_setup_rollback_but_does_not_relabel_failed_observation_as_complete(tmp_path, monkeypatch):
...
def test_failure_saving_acknowledged_setup_keeps_in_memory_proof_for_rollback(monkeypatch):
    shadow, users, documents, _ = fixture(monkeypatch)
    def fail(kind, **_fields):
        if kind == "document-created": raise OSError("journal-full")
    monkeypatch.setattr(shadow, "_record_setup", fail)
    with pytest.raises(OSError): shadow.setup()
    result = shadow.recover_setup()
    assert result["complete"] is True
    assert not users and not documents


@pytest.mark.parametrize("kind", ["local-shadow.json", "setup-journal.jsonl", "journal.jsonl", "child-started"])
def test_child_refuses_existing_run_evidence_before_any_api_effect(tmp_path, monkeypatch, kind):
    (tmp_path / kind).write_text("original")
    monkeypatch.setenv("FIRESTORE_EMULATOR_HOST", "127.0.0.1:18081")
    monkeypatch.setenv("FIREBASE_AUTH_EMULATOR_HOST", "127.0.0.1:18082")
    monkeypatch.setattr(module, "LocalShadow", lambda *_a: pytest.fail("API context constructed"))
    with pytest.raises((module.Refused, FileExistsError)): module.run_child(tmp_path, NONCE)
