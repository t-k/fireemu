...
const closurePath = fileURLToPath(
  new URL("../../spec/compatibility/closure/AUTH-ACTION.json", import.meta.url),
);

// The frozen AUTH-ACTION inventory. Adding or removing a row requires a new production
// mismatch or an uncovered acceptance requirement, and an edit here in the same commit.
const requiredConditions = new Set([
  "AUTH-ACTION/admin-link-generation",
  "AUTH-ACTION/client-send-boundary",
  "AUTH-ACTION/password-reset",
  "AUTH-ACTION/verify-email",
  "AUTH-ACTION/verify-and-change-email",
...
// Every request type production knows: each is generated through the Admin route at least once.
const requestTypes = ["PASSWORD_RESET", "VERIFY_EMAIL", "VERIFY_AND_CHANGE_EMAIL", "EMAIL_SIGNIN"];
...
test("AUTH-ACTION closure inventory cannot silently omit a declared condition", () => {
  const closure = load();
  assert.equal(closure.parent, "AUTH-ACTION");
  ...
  for (const condition of closure.conditions) {
    ...
    const off = rows
      ...
      .map(({ row }) => row);
    if (label === "AUTH-ACTION/closure-review") {
    ...
    } else if (label === "AUTH-ACTION/final-artifact-regression") {
...
test("closure recipes and corpus programs cover each other", async () => {
  const { PROGRAMS } = await import("./auth-action/corpus.mjs");
  ...
  const own = recipes.filter((id) => id.startsWith("auth-action/"));
...
test("the Admin generation program asks for every request type", async () => {
  const { PROGRAMS } = await import("./auth-action/corpus.mjs");
  const generation = PROGRAMS.find(({ id }) => id === "auth-action/generate/admin");
  const asked = new Set(generation.steps.map(({ body }) => body?.requestType).filter(Boolean));
  for (const type of requestTypes) assert.ok(asked.has(type), type);
});

test("only the email-link programs switch the config, and only the email-link switch", async () => {
  const { PROGRAMS } = await import("./auth-action/corpus.mjs");
  for (const { id, config, steps } of PROGRAMS) {
    const links = steps.some(({ path }) => path.endsWith("signInWithEmailLink"));
    if (!config) {
      assert.ok(!links, `${id} signs in by link without switching email links on`);
      continue;
    }
    assert.deepEqual(config, { "signIn.email.passwordRequired": false }, id);
  }
});
