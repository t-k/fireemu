...
The real `firebase-admin`, `firebase` (Node: gRPC streams; browser: the WebChannel transport on the same port), and `firebase/firestore/lite` (REST) SDKs run against the daemon; `tools/sdk-smoke` holds the smoke scripts and a browser page. Not implemented yet: `PartitionQuery`, `ExecutePipeline`, Storage triggers / Functions / Scheduler (Milestone C), signed ID tokens, inequality constraints in query rules proofs.
