...
def observe(output, origin=None):
    ...
    try:
        ...
        def observe_photo(name, status, value, unchanged):
            report["cases"].append(photo_row(name, status, photo(value), unchanged))

        # state() preserves key presence for photoUrl so absence is not collapsed to null.
        observe_photo("initial-lookup", status, initial, True)
        for name, url in (("set-photo", FIRST), ("replace-photo", SECOND)):
            status, value = client("update", {"idToken": token, "photoUrl": url})
            observe_photo(
                name,
                status,
                value,
                ...
                and value.get("displayName") == marker,
            )
            status, current = state(token)
            observe_photo(name + "-lookup", status, current, identity_matches(current))
        _, prior = state(token)
        status, value = client(
            "update", {"idToken": "not-a-valid-id-token", "photoUrl": REFUSED}
        )
        record(
        ...
        status, value = client(
            "update", {"idToken": token, "deleteAttribute": ["PHOTO_URL"]}
        )
        observe_photo(
            "delete-photo",
            status,
        ...
        status, current = state(token)
        observe_photo(
            "deleted-photo-lookup", status, current, identity_matches(current)
        )
