...
- Both profiles: a password reset or verification code acts on the account that owns its address now, also when another account took the address after the code was issued, as production does (sandbox recording 2026-09-25, `auth-action/address-reuse`) and as the official emulator does. The emulator profile now answers a code whose address nobody owns with the official `INVALID_OOB_CODE` and spends it, as the official emulator does; it used to apply the code to the account it was issued for.
