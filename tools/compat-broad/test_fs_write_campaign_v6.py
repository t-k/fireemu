...
CURRENT_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v6.json"
CURRENT_BINDING = PACKAGE_DIR / "fs-write-txn-precedence-01-v6-binding.json"
CURRENT_SHADOW = PACKAGE_DIR / "fs-write-txn-precedence-01-v6-local-shadow.json"
PREVIOUS_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v5.json"

EXPECTED_HEAD = "97ef1d3a8fb52938bbe26b1da9c85b9c5b430dd3"
EXPECTED_ARTIFACT = "d8b6e59ce625e4593eaf05fed7394f4e2c561a72efdecd39977de4115761d8cc"
...
def test_v6_is_a_new_immutable_package_bound_to_current_head() -> None:
    ...
    assert manifest["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V6"
    assert binding["campaignId"] == manifest["campaignId"]
    assert shadow["campaignId"] == manifest["campaignId"]
    assert manifest["preparation"]["supersedes"] == "FS-WRITE-TXN-PRECEDENCE-01-V5"
    assert previous["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V5"
...
def test_v6_artifact_and_manifest_bindings_are_consistent() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
    ...
    assert manifest["sourceBinding"]["artifactBuild"]["relativePath"] == "target/o3-fs-write-v6/release/fireemu"
    assert binding["artifactBuild"]["relativePath"] == "target/o3-fs-write-v6/release/fireemu"
    assert shadow["artifactBuild"]["relativePath"] == "target/o3-fs-write-v6/release/fireemu"
    ...
    assert binding["manifest"] == {
        "path": "spec/compatibility/broad-runs/fs-write-txn-precedence-01-v6.json",
...
def test_v6_remains_blocked_without_owner_or_stream_comparator() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
