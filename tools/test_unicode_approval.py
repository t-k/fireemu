...
def load():
    spec = importlib.util.spec_from_file_location(
        "unicode_approval",
        Path(__file__).with_name("auth-password-unicode-recheck-approval.py"),
    )
...
def test_human_approval_matches_complete_subject():
    ...
    assert "8 input patterns verified and approved" in page
...
def test_changed_observation_cannot_inherit_approval(field):
    ...
    if field == "artifact":
        ...
        receipt["observation"]["local"]["cases"][0]["checks"]["postLookup"] = False
    with pytest.raises(ValueError):
