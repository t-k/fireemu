...
def observe(output, origin=None):
    ...
    def inspect_call(
        ...
        if scheduled:
            ...
            row["quality"] = invalid_control_quality(projected)
        else:
            row["quality"] = (
                "observed"
    ...
    try:
        if production:
            ...
            password_policy(200, PASSWORD_POLICY, (password,))
        ...
        reference = baseline(
            "reference-refresh",
            lambda: exchange(frozen["a-refresh"]),
            frozen["a-refresh"],
        )
        # Origin is receipt completion; reference start/end are retained separately.
        sampling_origin = time.monotonic()
        reference_end = elapsed(begun)
        reference_start = report["cases"][-1]["startMs"]
        reference_time = report["cases"][-1]["response"]["tokenTime"]
        report["timing"] = {
            "sessionsSeparated": True,
            "issueGapMs": report["cases"][2]["startMs"]
            - report["cases"][1]["primaryEndMs"],
            "preReferenceWaitMs": reference_start - report["cases"][8]["endMs"],
            "referenceStartMs": reference_start,
            "referenceEndMs": reference_end,
            "latestPreIssuedAt": latest,
            "referenceIssuedAt": reference_time["iat"],
            "orderEstablished": reference_time["iat"] > latest,
        }
        require(report["timing"]["orderEstablished"])
        credentials = MappingProxyType(
            {
                **frozen,
                "reference-id": reference["id_token"],
                "reference-refresh": reference["refresh_token"],
            }
        )
        ...
        # Final original-password signin is outside the observation window.
        ...
        report["cases"].append(row)
        for name, invalid in (
            ("malformed-refresh", "malformed.token"),
            ("unknown-refresh", "rt-unknown-control"),
        ):
            row, _ = inspect_call(
                name, lambda value=invalid: exchange(value), begun, invalid
            )
            report["cases"].append(row)
        owned(lookup({"localId": [uid]}), email, marker, uid)
        row, _ = inspect_call(
