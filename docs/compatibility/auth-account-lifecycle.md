...
Provider metadata follows the live credential state during this lifecycle. IdP account recycling and explicit password removal select a surviving federated, phone or anonymous provider instead of retaining a stale `password` classification. A password account whose hash is not exportable remains classified as `password` across export/import/re-export; the regression coverage is in `crates/fireemu/src/import_export.rs` and `crates/fireemu/tests/import_export.rs`.
