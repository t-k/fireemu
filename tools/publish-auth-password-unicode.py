...
sys.path.insert(0, str(ROOT / "tools/auth-password-unicode"))
sys.path.insert(0, str(ROOT / "tools/compat-inventory"))
from evidence_common import runtime_inputs
from unicode_contract import CASES, SHAPES, complete, hypotheses, require, validate_case
from unicode_recorder import PASSWORD_POLICY, digest, inputs

BUNDLE = ROOT / "spec/compatibility/evidence/auth-password-unicode/receipt.json"
REVIEW = ROOT / "spec/compatibility/evidence/auth-password-unicode/source-review.json"
PAGE = ROOT / "docs/compatibility/auth-password-unicode.md"
INPUT_SHAPE = SHAPES
SCOPE = "Eight generated password upper-bound inputs, each with a distinct dedicated account, under the recorded minimum 6 / maximum 4096 policy. End-user REST, no tenant, owned strict artifact versus production. Redacted observations of acceptance/refusal plus branch-specific credential/state controls; no human approval, no universal Unicode rule or SDK/Rules/expiry claim."
CORPUS = {"slice": "auth-password-unicode", "revision": 1, "cases": list(CASES)}
...
def render(value):
    ...
    for target in ("local", "production"):
        ...
        lines.append(
            f"{target}: counting hypotheses consistent with these eight outcomes: {', '.join(fits) or 'none'}."
        )
    lines.extend(
        [
            "",
