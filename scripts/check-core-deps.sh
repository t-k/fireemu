#!/usr/bin/env bash
# CI 31.1 #5: every ftd-core-* crate must have zero normal external dependencies (ADR-001).
# Path dependencies on other ftd-core-* crates are allowed; everything else fails.
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."
status=0
for manifest in crates/ftd-core-*/Cargo.toml; do
...
for d in pkg["dependencies"]:
    if d["kind"] is None and not d["name"].startswith("ftd-core-"):
        print(d["name"])
' "$crate")"
  if [[ -n "$deps" ]]; then
