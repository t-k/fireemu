...
def publisher():
    path = Path(__file__).resolve().parents[1] / "publish-auth-deleted.py"
    spec = importlib.util.spec_from_file_location("deleted_publisher", path)
...
def test_completed_flow_is_candidate_not_universal_revocation():
    ...
    assert len(value["local"]["cases"]) == 12
...
def test_invalid_evidence_is_not_publishable(mutation):
    ...
    if mutation == "secret":
        ...
        local["transitions"][0]["uidAbsent"] = 1
    elif mutation == "setup":
        local["setup"]["a"] = 1
    elif mutation == "exit":
...
def test_publisher_exists():
    assert (Path(__file__).resolve().parents[1] / "publish-auth-deleted.py").exists()
