"""Publish the local-versus-production comparison of the readback-timing corpus.
...
RECEIPT = ROOT / "spec/compatibility/evidence/auth-blocking-readback/receipt.json"
BUNDLE = (
    ROOT / "spec/compatibility/evidence/auth-blocking-readback/local-comparison.json"
)
PAGE = ROOT / "docs/compatibility/auth-blocking-readback-comparison.md"
...
FIXTURE_FILES = (
    ...
    "tools/auth-blocking-disable/function-local/package-lock.json",
)
...
def project_local(report, recorder_commit, runtime_commit):
    ...
    require(
        ...
        == digest(
            {"slice": "auth-blocking-readback", "revision": 1, "cases": list(CASES)}
        )
        and complete(report)
    )
...
if __name__ == "__main__":
    ...
    print("Auth blocking readback comparison checked; subject " + digest(value))
