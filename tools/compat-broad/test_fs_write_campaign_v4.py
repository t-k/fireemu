...
CURRENT_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v4.json"
CURRENT_BINDING = PACKAGE_DIR / "fs-write-txn-precedence-01-v4-binding.json"
CURRENT_SHADOW = PACKAGE_DIR / "fs-write-txn-precedence-01-v4-local-shadow.json"
PREVIOUS_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v3.json"

EXPECTED_HEAD = "c4883258c06d769cec9d3e2081c2160bd4794ba2"
...
def test_v4_is_a_new_immutable_package_bound_to_current_head() -> None:
    ...
    assert manifest["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V4"
    assert binding["campaignId"] == manifest["campaignId"]
    assert shadow["campaignId"] == manifest["campaignId"]
    assert manifest["preparation"]["supersedes"] == "FS-WRITE-TXN-PRECEDENCE-01-V3"
    assert previous["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V3"
...
def test_v4_artifact_and_manifest_bindings_are_consistent() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
    ...
    assert manifest["sourceBinding"]["artifactBuild"]["relativePath"] == "target/o3-fs-write-v4/release/fireemu"
    assert binding["artifactBuild"]["relativePath"] == "target/o3-fs-write-v4/release/fireemu"
    assert shadow["artifactBuild"]["relativePath"] == "target/o3-fs-write-v4/release/fireemu"
    ...
    assert binding["manifest"] == {
        "path": "spec/compatibility/broad-runs/fs-write-txn-precedence-01-v4.json",
...
def test_v4_remains_blocked_without_owner_or_stream_comparator() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
