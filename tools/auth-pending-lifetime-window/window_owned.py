"""Build and own the strict Auth artifact; sample 300/450/600 seconds using its
virtual clock. Refusals trigger same-account controls. Never attach to an external daemon."""

# ruff: noqa: BLE001 -- Always reap owned processes and sanitize window failures.
import argparse
import json
import os
import shutil
...
def owned_complete(report):
    """An owned local run is complete only when the corpus contract is satisfied, the run
    aged by the virtual clock, and the owned child process was confirmed stopped."""
    return (
        complete(report)
        and report.get("agingMode") == "virtual-clock"
        and report.get("target") == "local"
        and "childCleanupFailure" not in report
    )
...
def child(directory, nonce):
    ...
    return observe(directory / "observation", origin, clock_control=(control, token))
...
def run(output):
    ...
    with tempfile.TemporaryDirectory(
        prefix="fireemu-pending-lifetime-window-owned-"
    ) as temporary:
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise SystemExit(
            "Pending-lifetime window owned observation did not complete"
        ) from None
