...
import { CASE, CASES, COMMIT_TRANSFORM_CASE, G0_CASE, TRANSFORMS_CASE, PRECONDITIONS_CASE, selectCase } from "../registry.mjs";
...
test("registry exposes registered cases and rejects an unknown id", () => {
  assert.deepEqual(
    CASES.map((c) => c.id),
    [CASE.id, COMMIT_TRANSFORM_CASE.id, G0_CASE.id, TRANSFORMS_CASE.id, PRECONDITIONS_CASE.id],
  );
