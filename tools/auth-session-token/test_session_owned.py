...
@pytest.mark.skipif(
    os.getenv("AUTH_SESSION_TOKEN_LIVE_LOCAL") != "1", reason="owned process opt-in"
)
def test_session_owned_lifecycle(tmp_path):
    path = Path(__file__).with_name("session_owned.py")
    ...
    from session_contract import complete

    assert complete(value)
    assert len(value["cases"]) == 32
