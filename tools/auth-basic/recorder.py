...
def observe(output, origin=None):
    ...
    try:
        ...
        record("signup", status, {"httpOk": status == 200, **checks})
        require(status == 200 and all(checks.values()))
        status, signin = client(
            "signInWithPassword",
            {"email": email, "password": password, "returnSecureToken": True},
        )
        checks = tokens(signin, uid, email)
        record("signin", status, {"httpOk": status == 200, **checks})
        require(status == 200 and all(checks.values()))
        ...
        record("refresh", status, {"httpOk": status == 200, **checks})
        require(status == 200 and all(checks.values()))
