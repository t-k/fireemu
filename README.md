...
- where production and the official emulator disagree, fireemu follows production and records the difference in `conformance/divergences.json`. Three examples: an equality filter combined with an inequality on another field is refused without a composite index, as production does; a REST `runQuery` response omits the `done` flag that the official emulator adds; and under `strict` a request against a Firestore database that was never created answers `NOT_FOUND` as production does, where the official emulator serves any syntactically valid database id. The default database and the databases a `firebase.json` `firestore` entry names are always served;
