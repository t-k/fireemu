...
pub fn capabilities_manifest() -> Value {
    json!({
        ...
        "capabilities": {
            ...
            "STORAGE-RULES-1": {"status": "implemented", "implemented": ["service firebase.storage: get / list / create / update / delete with request.resource and resource", "resumable uploads are authorized at finalization with the received bytes", "list requires rules_version 2", "Firebase protocol goes through the rules; the JSON API is a privileged surface unless an end-user token is presented", "firestore.get() / firestore.exists() over the latest Firestore state of the project"]},
        }
    })
}
