...
/// FS-WRITE-STREAM-LOCAL: one empty Write entry after the handshake is ignored when the client
/// immediately half-closes, completing the stream successfully.
#[tokio::test]
async fn write_stream_empty_write_entry_then_half_close_completes_successfully() {
    let (mut client, handle) = start(false).await;
    let (tx, rx) = mpsc::channel(8);
    let mut responses = client
        .write(ReceiverStream::new(rx))
    ...
    let mut open_stream = client
        .write(ReceiverStream::new(open_rx))
        .await
        .unwrap()
        .into_inner();
    open_tx
        ...
        .await
        .unwrap();
    let open_handshake = open_stream.next().await.unwrap().unwrap();
    open_tx
        .send(pb::WriteRequest {
            stream_token: open_handshake.stream_token.clone(),
            writes: vec![pb::Write::default()],
            ..Default::default()
        })
        .await
    ...
    open_tx
        .send(pb::WriteRequest {
            stream_token: open_handshake.stream_token,
            writes: vec![set_write("stream/after-empty", &[("v", s("x"))])],
            ..Default::default()
        })
        .await
        .unwrap();
    assert_eq!(
        open_stream.next().await.unwrap().unwrap_err().code(),
        tonic::Code::InvalidArgument
    );
    drop(open_tx);
    drop(open_stream);
    assert!(client
        .get_document(pb::GetDocumentRequest {
            name: format!("{DOCS}/stream/after-empty"),
            ..Default::default()
        })
        .await
        .is_err());

    let (malformed_tx, malformed_rx) = mpsc::channel(8);
    let mut malformed = client
        .write(ReceiverStream::new(malformed_rx))
        .await
        .unwrap()
        .into_inner();
    malformed_tx
        ...
        .await
        .unwrap();
    let malformed_handshake = malformed.next().await.unwrap().unwrap();
    malformed_tx
        .send(pb::WriteRequest {
            stream_token: malformed_handshake.stream_token,
            writes: vec![pb::Write {
                current_document: Some(pb::Precondition::default()),
                ..Default::default()
            }],
            ..Default::default()
        })
        ...
        .unwrap();
    assert_eq!(
        malformed.next().await.unwrap().unwrap_err().code(),
        tonic::Code::InvalidArgument
    );
    drop(malformed_tx);
    drop(malformed);
    ...
    let commit_error = client
        ...
        .await
        .unwrap_err();
    assert_eq!(commit_error.code(), tonic::Code::InvalidArgument);
    handle.abort();
    handle.await.unwrap_err();
}
