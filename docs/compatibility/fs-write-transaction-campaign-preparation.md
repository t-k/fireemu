...
The current package prepares the next bounded Oracle campaign for `FS-DATA-WRITE` and `FS-TRANSACTION` at feature head `9830ccaf53def5d51bb620411648e2a30337a32f`. It is preparation and local-shadow evidence only. It does not contain a production receipt, does not authorize a Cloud request and does not promote either parent group. The earlier `FS-WRITE-TXN-PRECEDENCE-01` package remains preserved as an immutable historical binding to the earlier source and artifact.

The current machine-readable package is [`fs-write-txn-precedence-01-v9.json`](../../spec/compatibility/broad-runs/fs-write-txn-precedence-01-v9.json). Its companion binding is [`fs-write-txn-precedence-01-v9-binding.json`](../../spec/compatibility/broad-runs/fs-write-txn-precedence-01-v9-binding.json), and the local-only shadow plan is [`fs-write-txn-precedence-01-v9-local-shadow.json`](../../spec/compatibility/broad-runs/fs-write-txn-precedence-01-v9-local-shadow.json). The historical package and v2/v3/v4/v5/v6/v7/v8 packages remain preserved as immutable bindings; v9 is the current-head package after the Auth settings integration. The `codeSourceHead` in the package identifies the repository revision used to build the artifact; the package/evidence commit that adds this rebind is tracked separately in Git and is intentionally not embedded in its own digest-bound JSON.

## Current, next and backlog

| Queue position | Campaign | Scope | State |
| --- | --- | --- | --- |
| CURRENT | `FS-WRITE-TXN-PRECEDENCE-01-V9` | A Write stream request contending with an active read-write transaction, followed by owned document-size and nesting controls | `BLOCKED_OWNER` and `BLOCKED_TECHNICAL` |
