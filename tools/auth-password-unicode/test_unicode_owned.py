...
@pytest.mark.skipif(
    os.getenv("AUTH_PASSWORD_UNICODE_LIVE_LOCAL") != "1", reason="owned process opt-in"
)
def test_unicode_owned_lifecycle(tmp_path):
    path = Path(__file__).with_name("unicode_owned.py")
    ...
    from unicode_contract import CASES, complete, validate_case
    from unicode_recorder import core
