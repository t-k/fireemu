...
//!
//! No decision is taken here: the module parses the transport, asks `fireemu-core-app-check`, and
...
use fireemu_core_app_check::limits::MAX_EXCHANGE_BODY_BYTES;
use fireemu_core_app_check::observe::{CredentialCategory, Observation, UNKNOWN_APP_LABEL};
use fireemu_core_app_check::registry::{
    validate_display_name, AppCheckRegistry, DebugTokenDigest, RegistryError,
};
use fireemu_core_app_check::verify::{AppCheckFailure, BaselineMode};
...
impl AppCheckState {
    ...
    #[must_use]
    pub fn gate(&self) -> fireemu_core_app_check::admission::AppCheckGate {
        fireemu_core_app_check::admission::AppCheckGate::new(
            self.registry.clone(),
            self.signer.clone(),
        )
    }
...
fn exchange_debug_token(
    ...
    let (response, observation) = match outcome {
        ...
        ExchangeOutcome::Issued(claims) => {
            let token = fireemu_core_app_check::jwt::encode(&claims, state.signer.as_ref());
            (
                JsonResponse {
                    status: 200,
