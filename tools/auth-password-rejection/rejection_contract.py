...
CASES = (
    ...
    "baseline-signin",
    "baseline-refresh",
    "baseline-refreshed-lookup",
    "weak-password-rejected",
    "unchanged-state",
    "original-password-signin",
    "original-password-lookup",
    "original-token-refresh",
    "original-refreshed-lookup",
...
REFRESH_CASES = {"baseline-refresh", "original-token-refresh"}
...
CHECKS = {
    ...
    **{
    ...
    "weak-password-rejected": {"rejected", "expectedError"},
    "delete": {"httpOk", "noError"},
    "deleted-account-absent": {"bothSelectorsAbsent"},
}


def rejection(status, value):
    code = error_code(value)
    checks = {"rejected": status == 400, "expectedError": code == "WEAK_PASSWORD"}
    return {
        "id": "weak-password-rejected",
        "httpStatus": status,
        "observedError": code,
        "checks": checks,
...
def validate_case(row, name):
    ...
    extra = (
        ...
        else ({"observedError"} if name == "weak-password-rejected" else set())
    )
    ...
    expected_status = 400 if name == "weak-password-rejected" else 200
    ...
    if name == "weak-password-rejected":
        code = row["observedError"]
        require(isinstance(code, str) and code in ERROR_CODES)
        require(checks["expectedError"] == (code == "WEAK_PASSWORD"))
