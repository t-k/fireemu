...
@pytest.mark.skipif(
    os.getenv("AUTH_SESSION_CONTINUITY_LIVE_LOCAL") != "1",
    reason="owned process opt-in",
)
def test_continuity_owned_lifecycle(tmp_path):
    path = Path(__file__).with_name("continuity_owned.py")
    ...
    from continuity_contract import complete
