...
DEADLINE_MS = 45000
LANES = ("a-id", "a-refresh", "b-id", "b-refresh", "changed-id", "changed-refresh")
...
def sample_quality(primary, followup, start, end, target, lane):
    valid = {"accepted", "auth-rejected"}
    if primary["outcome"] not in valid:
        return "inconclusive"
    if (
        "refresh" in lane
        and primary["outcome"] == "accepted"
        and (followup is None or followup["outcome"] not in valid)
    ):
        return "inconclusive"
    if lane.startswith("changed") and (
        primary["outcome"] != "accepted"
        or (
            "refresh" in lane
            and (followup is None or followup["outcome"] != "accepted")
        )
    ):
        return "inconclusive"
