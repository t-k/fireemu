...
def load_tool():
    path = ROOT / "auth-password-approval.py"
    assert path.exists(), "The subject-preserving approval overlay must exist"
    spec = importlib.util.spec_from_file_location("auth_password_approval", path)
...
def test_approved_overlay_is_current_and_scoped():
    ...
    assert "Old-token revocation timing" in page
    assert "fixed corpus names" not in page
    assert "Five token-returning cases" in page
