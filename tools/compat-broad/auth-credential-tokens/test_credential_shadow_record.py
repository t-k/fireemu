...
SUPERSEDED = ROOT / "spec/compatibility/broad-runs/auth-credential-tokens-local-shadow-20260918.json"
...
def test_the_record_is_a_complete_local_shadow_of_every_case() -> None:
    ...
    assert receipt["cleanup"] == {"ownedAccounts": 3, "remainingAccounts": 0, "addressReadbacks": 2, "cleanupComplete": True}
    assert [row["caseId"] for row in receipt["rows"]] == [case["id"] for case in observation_cases()]
    assert shadow._agreement(receipt["rows"])["unexpected"] == []
    assert record["shutdown"]["processStopped"] is True and record["shutdown"]["remainingChildren"] == 0
...
def test_the_record_carries_no_personal_path_or_credential() -> None:
    ...
    for fragment in ("/Users/", "/home/", "/private/tmp/", "/var/folders/", "Bearer ", "eyJ"):
...
def test_the_superseded_record_is_kept_byte_identical() -> None:
    ...
    assert superseded["receipt"]["collectorBinding"]["commit"] == "905ede564c6d40498b615183db9f4103756585ba"
