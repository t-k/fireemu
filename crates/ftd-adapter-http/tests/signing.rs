...
use ftd_adapter_http::identity_toolkit::{handle, AuthState, JWKS_PATHS};
use ftd_adapter_http::signing::RsaSigner;
...
fn session_rsa_tokens_round_trip_and_forgeries_are_refused() {
    ...
    let header: serde_json::Value =
        serde_json::from_slice(&ftd_core_auth::jwt::base64url_decode(parts[0]).unwrap()).unwrap();
    ...
    assert!(!other.verify(
        ...
        &ftd_core_auth::jwt::base64url_decode(parts[2]).unwrap()
    ));
    // Forgeries: a tampered payload, a foreign signature, and an unsigned token.
    let tampered = format!(
        "{}.{}.{}",
        parts[0],
        ftd_core_auth::jwt::base64url_encode(
            claims
                .canonical_json()
...
fn the_auth_surface_issues_signed_tokens_and_serves_the_jwks() {
    ...
    let unsigned = {
        ...
        format!(
            "{}.{}.",
            ftd_core_auth::jwt::base64url_encode(br#"{"alg":"none","typ":"JWT"}"#),
            parts[1]
        )
    };
    let forged = handle(
