...
CURRENT_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v5.json"
CURRENT_BINDING = PACKAGE_DIR / "fs-write-txn-precedence-01-v5-binding.json"
CURRENT_SHADOW = PACKAGE_DIR / "fs-write-txn-precedence-01-v5-local-shadow.json"
PREVIOUS_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v4.json"

EXPECTED_HEAD = "dcd58cea79b1571da082c9d3cceb06afb91b1865"
EXPECTED_ARTIFACT = "a1d2e1bf202183ab0f37ad01eb6d374bae602a04b4e2ceff0493bfc80f7ab64b"
...
def test_v5_is_a_new_immutable_package_bound_to_current_head() -> None:
    ...
    assert manifest["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V5"
    assert binding["campaignId"] == manifest["campaignId"]
    assert shadow["campaignId"] == manifest["campaignId"]
    assert manifest["preparation"]["supersedes"] == "FS-WRITE-TXN-PRECEDENCE-01-V4"
    assert previous["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V4"
...
def test_v5_artifact_and_manifest_bindings_are_consistent() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
    ...
    assert manifest["sourceBinding"]["artifactBuild"]["relativePath"] == "target/o3-fs-write-v5/release/fireemu"
    assert binding["artifactBuild"]["relativePath"] == "target/o3-fs-write-v5/release/fireemu"
    assert shadow["artifactBuild"]["relativePath"] == "target/o3-fs-write-v5/release/fireemu"
    ...
    assert binding["manifest"] == {
        "path": "spec/compatibility/broad-runs/fs-write-txn-precedence-01-v5.json",
...
def test_v5_remains_blocked_without_owner_or_stream_comparator() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
