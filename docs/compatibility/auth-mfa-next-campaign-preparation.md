...
Production time is wall-clock time. The descriptor refuses a simulated sleeper, the plan reference carries its timing mode, and the launcher refuses a rehearsal's frozen inputs. The 300, 450, 600 and 1800 second controls are real waits, checkpointed so a stopped process resumes for the remainder under the same Ledger reservation. Production execution is blocked on one thing: the shared Ledger caps a claim at 1200 s and releases only through the shared Gate's Firestore cleanup proof, and the Gate cannot host a 2700 s Auth campaign whose requests carry material from the previous response. The launcher names that refusal (`HostingRefused`) before reading a credential; it is a shared-module change, not a lane change.
