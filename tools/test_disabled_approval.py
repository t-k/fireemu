...
def load():
    spec = importlib.util.spec_from_file_location(
        "disabled_approval",
        Path(__file__).with_name("auth-disabled-recheck-approval.py"),
    )
    assert spec and spec.loader and Path(spec.origin).exists()
...
def test_human_approval_matches_complete_subject():
    ...
    assert "18 observations verified and approved" in page
    ...
    assert "all operation orders" in page
