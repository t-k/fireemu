"""Revision 3: finite MFA usability observations, never a proof of a common TTL.

Independent accounts sample 300/450/600 seconds. A verified completion supplies
an observed lower bound; later refusals supply measured, stage-specific candidates.
Age causality, account equivalence and monotonicity remain unproven. Revisions 1 and 2 stay pinned.
"""
...
CORPUS = {"slice": "auth-pending-lifetime", "revision": 3, "cases": list(CASES)}
...
def validate_timing(timing, name, outcome, skipped):
    ...
    if name.endswith("-state"):
        require(set(timing) == {"readbackSent", "readbackReceived"})
        _ordered(timing, ("readbackSent", "readbackReceived"))
        return
...
def validate_row(row, name):
    ...
    if row["skipped"]:
        require(
            name in DIAGNOSTIC
            and (name.endswith("-finalize") or "-post-refusal-" in name)
        )
    ...
    if name.endswith("-state"):
        require(set(row["checks"]) == STATE_CHECKS)
    ...
    if name.endswith("-start"):
        ...
        if "-post-refusal-" not in name:
            require(row["timing"]["pendingAgeAtStart"]["lower"] >= age_of(name))
        else:
            require(
                row["timing"]["pendingAgeAtStart"]["upper"] <= MAX_SESSION_AGE_SECONDS
            )
        return
...
def complete(report):
    try:
        ...
        for a in AGE_SECONDS:
            ...
            state = rows[f"age-{a}s-post-refusal-state"]
            fresh_start = rows[f"age-{a}s-post-refusal-fresh-start"]
            fresh_final = rows[f"age-{a}s-post-refusal-fresh-finalize"]
            refusal = start if start["outcome"] == "refused" else finalize
            refused = refusal["outcome"] == "refused"
            require(state["skipped"] == (not refused))
            healthy = (
                refused
                and state["outcome"] == "accepted"
                and all(state["checks"].values())
            )
            require(fresh_start["skipped"] == (not healthy))
            require(
                fresh_final["skipped"]
                == (
                    fresh_start["outcome"] != "accepted"
                    or fresh_start["checks"].get("sessionInfoPresent") is not True
                )
            )
            if refused:
                stage = "start" if refusal is start else "finalize"
                require(
                    refusal["timing"][stage + "Received"]
                    <= state["timing"]["readbackSent"]
                )
            if healthy:
                require(
                    state["timing"]["readbackReceived"]
                    <= fresh_start["timing"]["pendingSent"]
                )
            if not fresh_final["skipped"]:
                for key in START_ORDER:
                    require(fresh_final["timing"][key] == fresh_start["timing"][key])
        if report["target"] == "production":
            require(
...
def lifetime_summary(report):
    ...
    for a in refused:
        ...
        fresh_start = rows[f"age-{a}s-post-refusal-fresh-start"]
        fresh_final = rows[f"age-{a}s-post-refusal-fresh-finalize"]
        healthy = state["outcome"] == "accepted" and all(
            state["checks"].get(k) is True for k in STATE_CHECKS
        )
        controls.append(
            {
                "targetAgeSeconds": a,
                "accountStateMatches": healthy,
                "freshCompletionVerified": _verified_success(fresh_start, fresh_final),
                "ageCausalityEstablished": False,
            }
        )
    return {
        "sameAccountControls": controls,
        "usableAges": usable,
