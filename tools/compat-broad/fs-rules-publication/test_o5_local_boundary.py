...
def server():
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if self.path == "/short": status, raw, size = 404, b'{"error":{"status":"NOT_FOUND"}}', 900
            if self.path == "/oversize": raw = b'{"body":"' + b'x' * 65536 + b'"}'
            if self.path == "/broken": status, raw = 404, b'not-json'
            if self.path == "/array": raw = b'[]'
    ...
    try: yield f"http://127.0.0.1:{host.server_port}", calls
    finally: host.shutdown(); host.server_close(); thread.join(timeout=2); assert not thread.is_alive()
...
def test_malformed_http_cannot_become_a_complete_normalized_receipt(route):
    with server() as (origin, calls):
        with pytest.raises(module.Refused): module._request("GET", origin + "/" + route, credential="owner")
        assert len(calls) == 1


def test_redirect_is_not_followed_and_credential_is_not_replayed():
    with server() as (origin, calls):
        with pytest.raises(module.Refused): module._request("GET", origin + "/redirect", credential="owner")
...
@pytest.mark.parametrize("url", [
    "https://127.0.0.1:9000/path", "http://example.invalid:9000/path",
    "http://user@127.0.0.1:9000/path", "http://127.0.0.1:9000/path#fragment",
    "http://127.0.0.1/path", "http://127.0.0.1:0/path",
])
def test_nonlocal_or_ambiguous_url_refused_before_a_network_primitive(monkeypatch, url):
    def forbidden(*_a, **_k): raise AssertionError("network primitive reached")
    ...
    with pytest.raises(module.Refused): module._request("GET", url, credential="owner")


@pytest.mark.parametrize("body", [{}, {"users": None}, {"users": [] , "error": {"message": "INTERNAL"}},
    {"users": [{"localId": "wrong"}]}, {"users": [{"localId": "uid"}, {"localId": "uid"}]}])
def test_local_account_lookup_keeps_malformed_or_wrong_identity_unknown(monkeypatch, body):
    shadow = module.LocalShadow("http://127.0.0.1:9000", "http://127.0.0.1:9001", "p", "a"*32)
    shadow.plan = case(); entry = shadow.plan["ownedAccounts"][0]; shadow.uids[entry["ref"]] = "uid"
...
def test_lost_uid_is_not_proof_that_no_account_was_created():
    shadow = module.LocalShadow("http://127.0.0.1:9000", "http://127.0.0.1:9001", "p", "a"*32)
    shadow.plan = case(); entry = shadow.plan["ownedAccounts"][0]
    result = shadow._recover_account("account-readback", {"accountRef": entry["ref"]})
    assert result["complete"] is False
    assert result.get("accountPresent") is not False


@pytest.mark.parametrize("fault", ["recording", "cleanup", "deviation", "tenant", "setup"])
def test_child_never_exits_zero_for_failed_collection_state_or_recovery(tmp_path, monkeypatch, fault):
    class Shadow:
        tenant = None
        uids = {}
        wire_requests = 0
        ...
        def create_tenant(self): self.tenant = "tenant-local"; return self.tenant
        def publish(self, *_): pass
        def setup(self):
            if fault == "setup": raise module.Refused("fixture:failed")
        def execute(self, _): raise AssertionError("injected collector must not dispatch")
        def delete_tenant(self): Shadow.deleted += 1; return fault != "tenant"
    ...
    monkeypatch.setattr(module, "collect", lambda *_a, **_k: {
        "journal": "test-journal.jsonl", "recordingComplete": fault != "recording",
        "cleanup": {"cleanupComplete": fault != "cleanup"}, "rows": [],
    })
    monkeypatch.setattr(module, "local_deviations", lambda *_: [{"mismatch": True}] if fault == "deviation" else [])
    assert module.run_child(tmp_path, "b" * 32) != 0
    assert Shadow.deleted >= 1
    assert json.loads((tmp_path / "local-shadow.json").read_text())["completed"] is False
...
def test_local_account_absence_requires_a_consistent_complete_shape(monkeypatch, fault):
    ...
    body = {"users": []}
    if fault == "wrong-kind": body["kind"] = "unrelated"
    if fault == "page": body["nextPageToken"] = "continuation"
    monkeypatch.setattr(module, "_request", lambda *_a, **_k: (200, body))
...
@pytest.mark.parametrize("status,body", [
    (404, {}), (404, {"error": {"status": "NOT_FOUND"}}),
...
def test_document_lookup_never_fabricates_absence_or_ownership(monkeypatch, status, body):
    shadow = module.LocalShadow("http://127.0.0.1:9000", "http://127.0.0.1:9001", "p", "a" * 32)
...
def test_document_readback_and_typed_absence_are_preserved(monkeypatch, present):
    body = {"name": "resource", "updateTime": "2026-09-19T00:00:00.123456789Z"} if present else {
        "error": {"status": "NOT_FOUND", "code": 404, "message": "missing"}}
    shadow = module.LocalShadow("http://127.0.0.1:9000", "http://127.0.0.1:9001", "p", "a" * 32)
    monkeypatch.setattr(module, "_request", lambda *_a, **_k: (200 if present else 404, body))
    result = shadow._recover({"kind": "readback", "resource": "resource"})
    assert result["complete"] is True
    assert result["documentPresent"] is present


def test_child_can_succeed_with_complete_independent_collection_and_cleanup(tmp_path, monkeypatch):
    class Shadow:
        tenant = None
        uids = {}
        wire_requests = 0
        ...
        def setup(self): pass
        def execute(self, _): raise AssertionError("injected collector must not dispatch")
        def delete_tenant(self): Shadow.deleted += 1; return True
    monkeypatch.setattr(module, "LocalShadow", Shadow)
    monkeypatch.setenv("FIRESTORE_EMULATOR_HOST", "127.0.0.1:9000")
    monkeypatch.setenv("FIREBASE_AUTH_EMULATOR_HOST", "127.0.0.1:9001")
    monkeypatch.setattr(module, "collect", lambda *_a, **_k: {
        "journal": "test-journal.jsonl", "recordingComplete": True,
        "cleanup": {"cleanupComplete": True}, "rows": [],
    })
