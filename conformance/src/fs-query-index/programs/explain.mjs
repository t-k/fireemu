...
export const EXPLAIN_PROGRAMS = [
  {
    ...
    steps: [
      ...
      ...both(
        "range-descending",
        numbers({ where: f("n", "GREATER_THAN", int(3)), orderBy: [desc("n")] }),
      ),
      ...both(
        "composite",
        numbers({ where: and(f("g", "EQUAL", str("odd")), f("n", "GREATER_THAN", int(3))) }),
      ),
      ...both("merge", indexed({ where: and(f("a", "EQUAL", int(1)), f("b", "EQUAL", int(1))) })),
      ...both(
        "merge-with-order",
        indexed({
      ...
      ...both("or", numbers({ where: or(f("g", "EQUAL", str("odd")), f("h", "EQUAL", int(0))) })),
      ...both(
        "in",
        numbers({ where: f("n", "IN", { arrayValue: { values: [int(1), int(4), int(7)] } }) }),
      ),
      ...
      query("analyze-string", numbers(), { body: { explainOptions: { analyze: "true" } } }),
      query("options-unknown-field", numbers(), { body: { explainOptions: { verbose: true } } }),
      query(
