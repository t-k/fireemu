"""Publish the local-versus-production comparison of the blocking-disable corpus.
...
RECEIPT = ROOT / "spec/compatibility/evidence/auth-blocking-disable/receipt.json"
BUNDLE = (
    ROOT / "spec/compatibility/evidence/auth-blocking-disable/local-comparison.json"
)
PAGE = ROOT / "docs/compatibility/auth-blocking-disable-comparison.md"
...
def project_local(report, recorder_commit, runtime_commit):
    ...
    require(
        ...
        == digest(
            {"slice": "auth-blocking-disable", "revision": 1, "cases": list(CASES)}
        )
        and complete(report)
    )
...
if __name__ == "__main__":
    ...
    print("Auth blocking disable comparison checked; subject " + digest(value))
