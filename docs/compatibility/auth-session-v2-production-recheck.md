...
The current test path uses `tools/auth-session-v2/frozen_receipt.py` to validate an immutable receipt against its recorded probe and runtime snapshots without changing the historical publisher or approval source. Newly generated records continue to use the original strict publisher, while the frozen adapter retains the artifact, contract and mutation checks for the old receipt. This prevents later unrelated source changes from invalidating historical evidence.
