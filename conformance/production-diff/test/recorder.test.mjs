...
import { blobSha, compareRecords, digestJson } from '../core.mjs';
...
const upstream = process.env.PILOT_TEST_UPSTREAM_DIR ?? fileURLToPath(new URL('../../src/firestore-probe/', import.meta.url));
...
assert.equal(blobSha(sessionBytes), CASE.sessionBlob, 'recorder bytes must be the actual reviewed upstream bytes');
assert.equal(blobSha(credentialBytes), CASE.credentialsBlob);
const { compareProductionToFireemu: comparator } = await importText(comparatorModuleSource(
...
async function exercise(options = {}) {
  ...
  const rows = []; const docs = new Map(); let resets = 0;
  const server = createServer(async (req, res) => {
    let text = ''; for await (const part of req) text += part;
    rows.push({ path: req.url, method: req.method, authorization: req.headers.authorization ?? null });
    ...
    const error = (status, code) => send(status, { error: { code: status, status: code, message: 'fixture diagnostic' } });
    ...
    if (req.method === 'PATCH') {
      const document = { name, fields: body.fields, createTime: '2026-01-02T03:04:05Z', updateTime: '2026-01-02T03:04:05Z' };
      docs.set(name, document); return send(200, document);
    }
    if (req.method === 'GET') return docs.has(name) ? send(200, docs.get(name)) : error(404, 'NOT_FOUND');
    if (req.url.endsWith(':batchWrite')) {
      ...
      if (options.partialWrite) docs.set(body.writes[0].update.name, {
        ...body.writes[0].update, createTime: '2026-01-02T03:04:05Z', updateTime: '2026-01-02T03:04:05Z' });
  ...
  try {
    ...
    const program = programFixture(); if (options.inputDrift) program.steps[0].body.writes.pop();
    ...
    const processResult = await runProcess(process.execPath,
      ...
      { cwd: root, timeoutMs: 5000, env: { ...cleanEnvironment(root), PILOT_RUN_DIR: root,
        GOOGLE_CLOUD_PROJECT: CASE.project, FIRESTORE_EMULATOR_HOST: `127.0.0.1:${server.address().port}`,
        // Deliberately poisoned selectors: the wrapper must overwrite them before import.
        FIRESTORE_PROBE_TARGET: 'production', FIRESTORE_PROBE_HOST: 'firestore.googleapis.com',
        FIRESTORE_PROBE_TOKEN: 'sentinel-never-sent', FIRESTORE_PROBE_SCHEME: 'https',
      } });
    const session = JSON.parse(await fs.readFile(join(root, 'session-result.json')).catch(() => 'null'));
    const bytes = await fs.readFile(join(root, 'local.json')).catch(() => null);
    return { processResult, session, actual: bytes ? JSON.parse(bytes) : null, bytes, rows,
      compare: bytes ? compareRecords({ entry: CASE, program: programFixture(),
        production: selectProduction(matrixFixture(), CASE), actual: JSON.parse(bytes), comparator }) : null };
  } finally { server.closeAllConnections(); await new Promise(r => server.close(r));
    await fs.rm(root, { recursive: true, force: true }); }
}

test('actual legacy recorder completes 7 planned + 5 cleanup HTTP calls over loopback', async () => {
  const r = await exercise(); assert.equal(r.processResult.code, 0, r.processResult.log.toString());
  assert.equal(r.session.completed, true); assert.equal(r.rows.length, 12);
  assert.deepEqual(r.session.cleanup, { state: 'confirmed', absent: [...CASE.ownedDocuments], requests: 5 });
...
test('ambient production selectors do not send production credentials or paths', async () => {
  const r = await exercise(); assert.equal(r.processResult.code, 0);
  assert.ok(r.rows.every(row => row.authorization === null || row.authorization === 'Bearer owner'));
...
test('actual recorder + comparator detect a partial write despite the refusal', async () => {
  const r = await exercise({ partialWrite: true }); assert.equal(r.processResult.code, 0);
...
test('actual recorder + comparator detect a changed canonical error', async () => {
  const r = await exercise({ errorCode: 'FAILED_PRECONDITION' }); assert.equal(r.compare.verdict, 'MISMATCH');
});
test('reset failure does not become a valid recording', async () => {
  const r = await exercise({ resetFailure: true }); assert.notEqual(r.processResult.code, 0);
  assert.equal(r.session.completed, false); assert.equal(r.actual, null);
});
test('unconfirmed cleanup is an explicit failure', async () => {
  const r = await exercise({ cleanupFailure: true }); assert.notEqual(r.processResult.code, 0);
  assert.equal(r.session.cleanup.state, 'unconfirmed');
});
test('mutated program is rejected before any HTTP', async () => {
  const r = await exercise({ inputDrift: true }); assert.notEqual(r.processResult.code, 0);
  assert.equal(r.rows.length, 0); assert.equal(r.session, null);
});
test('local binding rejects a changed recording hash', async () => {
  const r = await exercise(); const b = Buffer.from(r.bytes.toString().replace('INVALID_ARGUMENT', 'PERMISSION_DENIED'));
  assert.throws(() => verifySession(r.session, { entry: CASE, program: programFixture() }, b), /local-record-binding/);
});
