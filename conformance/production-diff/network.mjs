...
export function assertUrl(value, origin) {
  let u; try { u = new URL(value); } catch { throw new Error('invalid-network-url'); }
  requireThat(u.origin === origin && u.hostname === '127.0.0.1' &&
    u.protocol === 'http:' && !u.username && !u.password && !u.hash, 'network-outside-owned-origin');
  return u;
}

export function installNetworkGuard(origin = null) {
  const deny = () => { throw new Error('offline-operation-denied'); };
  const connect = net.Socket.prototype.connect;
  net.Socket.prototype.connect = function (...args) {
    let a = args;
    if (Array.isArray(a[0])) a = a[0];
    let options = typeof a[0] === 'object' && a[0] !== null ? a[0]
      : { port: a[0], host: typeof a[1] === 'string' ? a[1] : 'localhost' };
    requireThat(origin && !options.path && options.host === '127.0.0.1' &&
      Number(options.port) === Number(new URL(origin).port), 'network-outside-owned-origin');
