...
CURRENT_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v3.json"
CURRENT_BINDING = PACKAGE_DIR / "fs-write-txn-precedence-01-v3-binding.json"
CURRENT_SHADOW = PACKAGE_DIR / "fs-write-txn-precedence-01-v3-local-shadow.json"
PREVIOUS_MANIFEST = PACKAGE_DIR / "fs-write-txn-precedence-01-v2.json"

EXPECTED_HEAD = "b31d088ed150c11d732ef596778f35605ad0078d"
EXPECTED_ARTIFACT = "e3d72d5a97d6e29bac5050d20655d45b581434af588ab760b55378a576bb41e5"
...
def test_v3_is_a_new_immutable_package_bound_to_current_head() -> None:
    ...
    assert manifest["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V3"
    assert binding["campaignId"] == manifest["campaignId"]
    assert shadow["campaignId"] == manifest["campaignId"]
    assert manifest["preparation"]["supersedes"] == "FS-WRITE-TXN-PRECEDENCE-01-V2"
    assert previous["campaignId"] == "FS-WRITE-TXN-PRECEDENCE-01-V2"
...
def test_v3_artifact_and_manifest_bindings_are_consistent() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
    ...
    assert binding["manifest"] == {
        "path": "spec/compatibility/broad-runs/fs-write-txn-precedence-01-v3.json",
...
def test_v3_remains_blocked_without_owner_or_stream_comparator() -> None:
    manifest = load_json(CURRENT_MANIFEST)
    binding = load_json(CURRENT_BINDING)
    shadow = load_json(CURRENT_SHADOW)
