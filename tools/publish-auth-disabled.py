...
sys.path.insert(0, str(ROOT / "tools/auth-disabled"))
...
from disabled_recorder import PASSWORD_POLICY, digest, inputs
from evidence_common import runtime_inputs

BUNDLE = ROOT / "spec/compatibility/evidence/auth-disabled/receipt.json"
REVIEW = ROOT / "spec/compatibility/evidence/auth-disabled/source-review.json"
PAGE = ROOT / "docs/compatibility/auth-disabled.md"
SCOPE = "Eighteen sequential REST route observations across baseline, disabled and re-enabled phases for target A and unaffected control B. Fixed initial ID/refresh tokens, password signin and derived-token lookup; no tenant, owned strict artifact and recorded production settings. Admin is used only for owned account state changes/readback and cleanup. No human approval, instant revocation, SDK, Rules or elapsed-expiry claim."
