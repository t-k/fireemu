...
pub fn capabilities_manifest() -> Value {
    json!({
        ...
        "capabilities": {
            ...
            "RULES-1": {"status": "partial", "precision": "conservative", "implemented": ["get / create / update / delete against the returned snapshot or the staged commit", "list / aggregation / Listen queries proven from equality and array-contains constraints (RULES-QUERY-CONSTRAINTS)", "collection groups need a recursive wildcard rule"], "unimplemented": ["getAfter()", "timestamp/duration/latlng/math/hashing namespaces", "inequality and request.query in proofs"], "notes": ["get()/exists() served from the snapshot being read (transaction / read_time included) with the RULES-DOC-ACCESS-SINGLE budget per operation and per query proof, and RULES-DOC-ACCESS-MULTI-TOTAL across atomic commits and BatchGetDocuments", "string.matches()/replace(): RE2-style subset without lookarounds, flags or word boundaries"]},
            ...
            "FN-EVT-1": {"status": "partial", "implemented": ["Firestore document created / updated / deleted / written triggers (v2, JSON CloudEvents)", "Storage object finalized / deleted / metadataUpdated triggers (v2)", "retries with virtual-time backoff for functions declared with retry", "per-function concurrency and global concurrency", "invocation timeouts"], "unimplemented": ["v1 (firebase-functions/v1) event and schedule functions", "withAuthContext attributes", "Pub/Sub, Realtime Database, Auth, Remote Config triggers", "protobuf CloudEvents payloads"]},
            "FN-SCH-1": {"status": "partial", "implemented": ["onSchedule with cron and App Engine schedules driven by the virtual clock (catch-up all, capped at 1000)", "manual runs via POST /v1/sessions/{s}/functions/{name}:run", "fixed-offset time zones"], "unimplemented": ["daylight-saving time zones", "overlap policies other than allow"]},
            "CTL-1-AWAIT-IDLE": {"status": "partial", "implemented": ["POST /v1/sessions/{s}:awaitIdle waits for pending / running / retry-waiting events, HTTP invocations and due schedule runs beyond the catch-up cap; events are enqueued inside the commit that produces them"], "unimplemented": ["causal fences across sessions", "Text Index work", "atomic reset across Firestore / Storage / functions while work is in flight"]},
