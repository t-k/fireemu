...
CASES = (
    ...
    "set-photo-lookup",
    "replace-photo",
    "replace-photo-lookup",
    "invalid-token-update",
    "unchanged-state",
    "delete-photo",
    "deleted-photo-lookup",
    "delete",
...
FIRST = "https://example.invalid/fireemu-profile-first.png"
SECOND = "https://example.invalid/fireemu-profile-second.png"
REFUSED = "https://example.invalid/fireemu-profile-refused.png"
PHOTO_EXPECTED = {
    "initial-lookup": "absent",
    "set-photo": "first",
    "set-photo-lookup": "first",
    "replace-photo": "second",
    "replace-photo-lookup": "second",
    "delete-photo": "absent",
    "deleted-photo-lookup": "absent",
}
PHOTO_STATES = {"absent", "null", "empty", "first", "second", "other", "invalid-type"}
TOKEN_CHECKS = set(tokens({}, "uid", "email")) | {"httpOk"}
CHECKS = {
    "signup": TOKEN_CHECKS,
    **{
        name: {"httpOk", "photoMatches", "selectedIdentityUnchanged"}
        for name in PHOTO_EXPECTED
    },
...
def photo(value):
    ...
    if not isinstance(item, str):
        return "invalid-type"
    return {"": "empty", FIRST: "first", SECOND: "second"}.get(item, "other")


def photo_row(name, status, observed, identity_unchanged):
    checks = {
        "httpOk": status == 200,
        "photoMatches": observed == PHOTO_EXPECTED[name],
...
def validate_case(row, name):
    ...
    extra = (
        {"expirySeconds"}
        if name == "signup"
        else ({"photoState"} if name in PHOTO_EXPECTED else set())
    )
    ...
    if name in PHOTO_EXPECTED:
        require(
            isinstance(row["photoState"], str) and row["photoState"] in PHOTO_STATES
        )
        require(checks["photoMatches"] == (row["photoState"] == PHOTO_EXPECTED[name]))
    if name == "signup":
