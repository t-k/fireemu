...
//!
//! ```text
//! GET  /ui, /ui/*                      the app (SPA fallback; index.html carries window.__FIREEMU__)
...
pub struct UiState {
    ...
    pub app_check: Option<Arc<fireemu_adapter_http::app_check::AppCheckState>>,
}
...
pub fn guard(state: &UiState, req: &UiRequest) -> Option<UiResponse> {
    ...
    if let Some(origin) = req.header("origin") {
        if !fireemu_adapter_http::identity_toolkit::origin_is_local(origin) {
    ...
    let presented = req
        ...
        .map(str::trim);
    if !fireemu_adapter_http::control::token_matches(presented, &state.control_token) {
        return Some(UiResponse::error(
            403,
...
fn script_nonce(state: &UiState) -> String {
    ...
    for b in state
        .control_token
        .bytes()
        .chain(b"fireemu-ui-nonce".iter().copied())
    {
...
/// The configuration the page receives (`window.__FIREEMU__`) and `GET /ui/api/config` returns.
#[must_use]
