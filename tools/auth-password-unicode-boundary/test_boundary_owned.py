...
@pytest.mark.skipif(
    os.getenv("AUTH_PASSWORD_UNICODE_BOUNDARY_LIVE_LOCAL") != "1",
    reason="owned process opt-in",
)
def test_boundary_owned_lifecycle(tmp_path):
    path = Path(__file__).with_name("boundary_owned.py")
    ...
    from boundary_contract import CASES, complete, validate_case
    from boundary_recorder import core
