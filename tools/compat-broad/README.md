...
Database evidence records separate full parsed-response and settings-projection hashes. Projection `database-settings-v1` excludes only `earliestVersionTime`; identity, edition, location, other settings, unknown fields and field presence/types remain bound. The same contract is used before and after execution. Google documents that `earliestVersionTime` continuously advances with the retention window, so its isolated movement is not configuration drift. [Database REST reference](https://docs.cloud.google.com/firestore/docs/reference/rest/v1/projects.databases) supplies this distinction. UID/edition/location changes still stop execution.
