...
- Strict profile: while client sign-up is off (`client.permissions.disabledUserSignup`), a phone code for a number and an email sign-in link for an address that no account holds are refused with `ADMIN_ONLY_OPERATION` when they are asked for, also from an administrator, as production refuses them. The emulator profile sends them, as the official emulator (which has no client permissions) does, and refuses the sign-in that would create the account.
