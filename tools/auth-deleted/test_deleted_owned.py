...
@pytest.mark.skipif(
    os.getenv("AUTH_DELETED_LIVE_LOCAL") != "1",
    reason="owned process opt-in",
)
def test_deleted_owned_lifecycle(tmp_path):
    path = Path(__file__).with_name("deleted_owned.py")
    ...
    from deleted_contract import CASES, complete, validate_row
    from deleted_recorder import core
