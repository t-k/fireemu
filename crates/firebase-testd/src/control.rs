...
pub fn capabilities_manifest() -> Value {
    json!({
        ...
        "capabilities": {
            ...
            "SCHED-1": {"status": "partial", "implemented": ["onSchedule with cron and App Engine schedules driven by the virtual clock (catch-up all, capped at 1000)", "manual runs via POST /v1/sessions/{s}/functions/{name}:run", "fixed-offset time zones"], "unimplemented": ["daylight-saving time zones", "overlap policies other than allow"]},
