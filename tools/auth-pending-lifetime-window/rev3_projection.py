...
def observation_auth_requests(age_outcomes: tuple[AgeOutcome, ...]) -> int:
    ...
    require(
        all(value in ("accepted", "start-refused", "finalize-refused") for value in age_outcomes),
...
def recovery_auth_requests(existing_accounts: int = MAX_ACCOUNTS) -> int:
    ...
    require(type(existing_accounts) is int and 0 <= existing_accounts <= MAX_ACCOUNTS,
...
def validate_budget() -> None:
    ...
    require(WALL_SECONDS == 1500 and RECOVERY_SECONDS == 300,
            "revision-3 wall/recovery envelope changed")
    require(MAX_ACCOUNTS == 5 and MAX_AUTH_REQUESTS == 300,
            "revision-3 account/request budget changed")
    require(RECOVERY_AUTH_RESERVE == 60, "recovery request reserve changed")
    require(observation_auth_requests(("start-refused",) * 3) <= OBSERVATION_AUTH_LIMIT,
            "refusal-path observation budget exceeded")
    require(recovery_auth_requests() <= RECOVERY_AUTH_RESERVE,
