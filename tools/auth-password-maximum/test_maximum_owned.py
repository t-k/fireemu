...
@pytest.mark.skipif(
    os.getenv("AUTH_PASSWORD_MAXIMUM_LIVE_LOCAL") != "1", reason="owned process opt-in"
)
def test_maximum_owned_lifecycle(tmp_path):
    path = Path(__file__).with_name("maximum_owned.py")
    ...
    from maximum_contract import CASES, complete, validate_case
    from maximum_recorder import recovery_identity
