...
def observe(output, origin=None):
    ...
    replacement = "Aa9!" + secrets.token_urlsafe(32)
    uid = None
    attempted = False
    ...
    try:
        ...
        def refresh_original(name, lookup_name):
        ...
        refresh_original("baseline-refresh", "baseline-refreshed-lookup")
        ...
        report["cases"].append(rejection(status, refused))
        observe_state("unchanged-state", baseline["idToken"], initial)
        status, original = client(
            "signInWithPassword",
            {"email": email, "password": password, "returnSecureToken": True},
        )
        observe_tokens("original-password-signin", status, original)
        observe_state("original-password-lookup", original["idToken"], initial)
        refresh_original("original-token-refresh", "original-refreshed-lookup")
        ...
        observe_tokens("valid-password-change", status, changed)
        observe_state("changed-token-lookup", changed["idToken"], initial)
