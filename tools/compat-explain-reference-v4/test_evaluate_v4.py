...
def private_inputs():
    ...
    base = Path(base)
    return base, {
        'production': base / 'docs.local/logs/2026-09-14/campaign-explain-production-109a9b45',
        'originalLocal': base / 'docs.local/logs/2026-09-14/campaign-explain-shadow-109a9b45',
        'currentLocal': base / 'docs.local/logs/2026-09-17/query-explain-shadow-cce4a4f9b-v4',
    }
...
def test_actual_saved_replay(private_inputs):
    ...
    result = module.evaluate(roots, base / '.worktree/campaign-explain-reference-109a9b45', base / '.worktree/query-current-collector-cce4a4f9b')
...
def test_actual_current_validator_rejects_control_tampering(private_inputs, tmp_path, control):
    module = load()
    base, roots = private_inputs
    ...
    with pytest.raises(ValueError, match='current validator refused'):
        module.current_validation(base / '.worktree/query-current-collector-cce4a4f9b', changed)
