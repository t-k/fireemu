...
def project_run(run: Path) -> dict[str, Any]:
    ...
    contract = compare_rows(
        ...
        plan,
        rows,
        left_recovery=cleanup,
        right_recovery=cleanup,
    )
...
def write_projection(run: Path, output: Path) -> None:
...
def main() -> int:
    ...
    write_projection(args.run.resolve(), args.output.resolve())
    return 0
