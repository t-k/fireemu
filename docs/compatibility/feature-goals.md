...
| --- | --- | --- | --- |
| G3-pending-switch-local | One pending SDK write during sign-out or A-to-B switch; distinguish the original write promise, cache state, authenticated server state and listener lifecycle | Existing owned SDK smoke; local invariants only until a production reference exists; Node gRPC only | queued; fix the finite operation sequence before dispatch |
| G4-select-copy-local | Measure wide-document projection copies with the same collection/select/limit path and fixed page sizes | Existing stats and executor; preserve alias/type/field semantics; no new stage, global quota or RSS claim | queued |
