...
from continuity_recorder import PROJECT, recovery_identity, save


def test_continuity_policy_validates_exactly_one_generated_password():
    from continuity_recorder import PASSWORD_POLICY, password_policy

    password = "Aa9!" + "a" * 43
    assert password_policy(200, PASSWORD_POLICY, (password,)) == PASSWORD_POLICY
    for passwords in ((), (password, password), ("short",)):
        with pytest.raises(ValueError):
            password_policy(200, PASSWORD_POLICY, passwords)
...
def test_cleanup_identity_is_persisted_and_rechecked_before_deletion(tmp_path):
    import continuity_recorder as recorder
