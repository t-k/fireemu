...
def contract():
    path = Path(__file__).with_name("unicode_contract.py")
    assert path.exists()
    spec = importlib.util.spec_from_file_location("unicode_contract", path)
...
def test_eight_actual_input_shapes_distinguish_counting_rules():
    c = contract()
