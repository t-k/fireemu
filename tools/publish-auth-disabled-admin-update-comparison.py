"""Publish the local-versus-production comparison of the disabled-account administrative
update corpus.
...
from admin_update_recorder import digest

RECEIPT = ROOT / "spec/compatibility/evidence/auth-disabled-admin-update/receipt.json"
BUNDLE = (
    ROOT
    / "spec/compatibility/evidence/auth-disabled-admin-update/local-comparison.json"
)
PAGE = ROOT / "docs/compatibility/auth-disabled-admin-update-comparison.md"
...
def project_local(report, recorder_commit, runtime_commit):
    ...
    require(
        ...
        == digest(
            {"slice": "auth-disabled-admin-update", "revision": 1, "cases": list(CASES)}
        )
        and complete(report)
    )
...
def validate_local(local):
    ...
    require(local["configurationUnchanged"] is True)
...
if __name__ == "__main__":
    ...
    print("Auth disabled admin update comparison checked; subject " + digest(value))
