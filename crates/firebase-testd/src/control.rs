...
pub fn capabilities_manifest() -> Value {
    json!({
        ...
        "capabilities": {
            "CTL-1": {"status": "partial", "implemented": ["sessions: POST /v1/sessions {project} creates a session isolated by project (own Firestore databases, default Storage buckets and Auth store; clock, rules, functions and fault plan are shared), GET lists, DELETE removes; project-scoped Auth routes and token verification pick the project's store", "clock:set", "clock:advance", "clock:advanceTo", "sessions/{s}/reset (exclusive across Firestore, Auth, Storage and functions)", "sessions/{s}/snapshots (POST {name} captures Firestore, Storage, Auth, the clock and both rulesets; GET lists; {name}:restore restores atomically under the reset barrier and resets the functions runtime; DELETE)", "sessions/{s}/faultPlan (PUT the spec 18.2 plan, GET plan + fired history + counters, DELETE)", "sessions/{s}/functions and functions/{name}:run", "sessions/{s}/pubsub/topics/{t}:publish", "sessions/{s}:awaitIdle", "rules load/replace/drop", "health", "capabilities", "limits"], "unimplemented": ["per-session clocks and functions (sessions share them)", "snapshot files on disk (snapshots live in memory for the daemon's lifetime)"], "notes": ["fault operations: firestore.commit / read / beginTransaction, storage.upload / read / delete / list, functions.deliver (duplicate), functions.invoke (returnError, timeout, deadLetter, delay, crashRunner); rules match the nth occurrence per operation, or per operation and function when a function is named"]},
