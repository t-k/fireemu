...
fn incoming_rules_value(
    ...
    if let Some((digest, crc)) = hashes {
        map.insert(
            "md5Hash".into(),
            s(&fireemu_core_storage::hash::base64(digest)),
        );
        map.insert(
            "crc32c".into(),
            s(&fireemu_core_storage::hash::base64(&crc.to_be_bytes())),
        );
    }
    map.insert(
        "contentType".into(),
...
impl StorageState {
    ...
    fn principal(&self, authorization: Option<&str>, bucket: &str) -> Result<Principal, String> {
        ...
        let aud = fireemu_core_auth::jwt::decode_token(token, store.signer())
            .ok()
            .and_then(|d| {
                d.payload
                    .get("aud")
