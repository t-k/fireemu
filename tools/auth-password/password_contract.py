...
CHECKS = {
    **{name: TOKEN_CHECKS for name in TOKEN_CASES if name != "changed-token-refresh"},
    "changed-token-refresh": set(tokens({}, "uid", "email", refresh=True)) | {"httpOk"},
    **{
    ...
    "old-password-rejected": {"rejected", "expectedError"},
...
def validate_case(row, name):
    ...
    extra = {"expirySeconds"} if name in TOKEN_CASES else set()
    ...
    expected_status = 400 if name == "old-password-rejected" else 200
