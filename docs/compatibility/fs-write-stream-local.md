...
The capability mapping is `FS-RPC-1` in `crates/fireemu/src/capabilities.json`. Active-stream acknowledgements are covered; closed-stream resumption is unsupported. Update-time preconditions and interaction with an active transaction are outside this finite artifact. `FS-WRITE` is mapped to `REQ-FS-PARITY-01` in the surface inventory as finite local evidence; production, REST and SDK parity remain explicit coverage debt. The generated inventory is `docs/compatibility/surfaces.md`; the broader Standard/Native feature mapping remains `FS-DATA` in `spec/compatibility/features.json`.
