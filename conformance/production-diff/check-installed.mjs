...
const actual = { [p.entry.programId]: { steps: Object.fromEntries(
...
const original = p.comparator({ production: p.production, fireemu: actual, programDefinitions: [p.program] });
assert.deepEqual(baseline.rows.map(row => row.comparison), original.rows.map(row => row.comparison.toUpperCase()));
...
const missing = structuredClone(actual); delete missing[p.entry.programId].steps['empty-batch'];
assert.equal(run(missing).verdict, 'INDETERMINATE');
const changedProgram = structuredClone(p.program); changedProgram.steps[0].body.writes.pop();
assert.throws(() => validateProgram(changedProgram, p.entry));
console.log(JSON.stringify({ caseId: p.entry.id, historicalComparatorCheck: 'passed',
  historicalRows: baseline.counts, semanticMutantsDetected: 2, missingRowRejected: true,
  changedInputRejected: true, productionExecuted: false, currentNativeExecuted: false,
  historicalArtifact: matrix.evidence.observations.fireemu.artifact,
  provenance: p.provenance }, null, 2));
